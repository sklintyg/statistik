...
public class HsaOrganizationsServiceImplTest {
  ...
  @Mock HsatkAuthorizationManagementService authorizationManagementService;

  @InjectMocks private HsaOrganizationsServiceImpl testee;

  @Test
  public void testWithSingleMiuStatistik() throws HsaServiceCallException {
    when(authorizationManagementService.getCredentialInformationForPerson(
            anyString(), anyString(), eq(null)))
        .thenReturn(buildCredzResponse(Medarbetaruppdrag.STATISTIK, 1, 1));
    List<Vardenhet> authorizedEnheter =
        testee.getAuthorizedEnheterForHosPerson(HSA_ID).getVardenhetList();
  ...
  private List<CredentialInformation> buildCredzResponse(
      String purpose, int number, int maxNumber) {
