...
/**
 * Interfaces with the {@link AuthorizationManagementService} in order to fetch Medarbetaruppdrag
 * for the given employeeHsaId. Will filter out any non "Statistik"-purposed MiU:s.
 *
 * @author erikl
 */
@Service
public class HsaOrganizationsServiceImpl implements HsaOrganizationsService {
  ...
  @Autowired private HsatkAuthorizationManagementService authorizationManagementService;

  @Autowired private HsatkOrganizationService organizationUnitService;
  ...
  public UserAuthorization getAuthorizedEnheterForHosPerson(HsaIdUser hosPersonHsaId) {
    ...
    try {
      response =
          authorizationManagementService.getCredentialInformationForPerson(
              "", hosPersonHsaId.getId(), null);
    } catch (Exception e) {
    ...
    if (response != null) {
      LOG.debug(
          "User with HSA-Id " + hosPersonHsaId + " has " + response.size() + " medarbetaruppdrag");

      for (CredentialInformation info : response) {
        vardenhetList.addAll(getAllVardenhetsWithMuWithStatistikPurpose(info));
        systemRoles.addAll(
            info.getHsaSystemRole().stream()
                .map(sr -> sr.getSystemId() + ";" + sr.getRole())
                .collect(Collectors.toList()));
      }
      vardenhetList =
          vardenhetList.stream().distinct().sorted(veComparator).collect(Collectors.toList());
    }
    LOG.debug(
        "User with HSA-Id "
            + hosPersonHsaId
            + " has active 'Statistik' for "
            + vardenhetList.size()
            + " enheter");
  ...
  private List<Vardenhet> getAllVardenhetsWithMuWithStatistikPurpose(CredentialInformation info) {
    ...
    for (Commission commissionType : info.getCommission()) {
      if (Medarbetaruppdrag.STATISTIK.equalsIgnoreCase(commissionType.getCommissionPurpose())) {
        enhets.add(
            new Vardenhet(
                new HsaIdEnhet(commissionType.getHealthCareUnitHsaId()),
                commissionType.getHealthCareUnitName(),
                new HsaIdVardgivare(commissionType.getHealthCareProviderHsaId()),
                commissionType.getHealthCareProviderName()));
      }
    }
    return enhets;
  }
