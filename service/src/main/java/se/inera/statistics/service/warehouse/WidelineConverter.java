...
public class WidelineConverter extends AbstractWidlineConverter {
  ...
  public List<WideLine> toWideline(
      IntygDTO dto, HsaInfo hsa, long logId, String correlationId, EventType type) {
  ...
  private WideLine createWideLine(
      long logId,
      ...
      Arbetsnedsattning arbetsnedsattning,
      boolean active) {
  ...
  private Diagnos parseDiagnos(String diagnos) {
    ...
    final Kategori kategori =
        kod != null ? kod.getKategori() : isUnknownDiagnos ? null : icd10.findKategori(diagnos);
  ...
  private void checkStartdatum(List<String> errors, int startdatum) {
    if (startdatum < DATE20100101
        || startdatum > toDay(LocalDate.now(clock).plusYears(MAX_YEARS_INTO_FUTURE))) {
