...
public final class KonDataResponses {
  ...
  public static KonDataResponse changeIdGroupsToNamesAndAddIdsToDuplicates(
      final KonDataResponse response, Map<? extends HsaIdAny, String> idsToNames) {
    final List<HsaIdAny> idsToCompare =
    ...
    for (Map.Entry<? extends HsaIdAny, String> entry : idsToNames.entrySet()) {
      hsaIdsToNames.put(
          HsaIdAny.createEvenThoughSubclassesArePreferred(entry.getKey().getId()),
          entry.getValue());
    }
    final HashMap<HsaIdAny, Integer> duplicatesPerId =
        getDuplicatesPerId(hsaIdsToNames, idsToCompare);
  ...
  private static HashMap<HsaIdAny, Integer> getDuplicatesPerId(
      Map<HsaIdAny, String> idsToNames, List<HsaIdAny> idsToCompare) {
    ...
    for (Map.Entry<CaseInsensiviteString, Collection<HsaIdAny>> entry :
        namesToIdsForIdsToCompare.asMap().entrySet()) {
