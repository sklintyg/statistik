...
public abstract class AbstractRegisterCertificateHelper
    extends IntygHelper<RegisterCertificateType> {

  private static final Logger LOG =
      LoggerFactory.getLogger(AbstractRegisterCertificateHelper.class);
  ...
  /**
   * Nothing that ST uses has been changed between v2 and v3 of the schema, therefore it should be
   * enough to use this simple transformation to make sure no v2 namespaces are still used.
   */
  public static String convertToV3(String data) {
    return data.replaceAll(
            "urn:riv:clinicalprocess:healthcond:certificate:2",
            "urn:riv:clinicalprocess:healthcond:certificate:3")
        .replaceAll(
            "urn:riv:clinicalprocess:healthcond:certificate:RegisterCertificateResponder:2",
            "urn:riv:clinicalprocess:healthcond:certificate:RegisterCertificateResponder:3")
        .replaceAll(
            "urn:riv:clinicalprocess:healthcond:certificate:types:2",
      ...
      if (o instanceof Node) {
        ...
        if (datePeriodType.getStart() == null || datePeriodType.getEnd() == null) {
          throw new ConverterException(
              "Error while converting DatePeriodType, missing mandatory field");
        }
