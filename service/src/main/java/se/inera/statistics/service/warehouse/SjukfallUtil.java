...
public class SjukfallUtil {
  ...
  public static final FilterPredicates ALL_ENHETER =
      new FilterPredicates(
          fact -> true, sjukfall -> true, FilterPredicates.HASH_EMPTY_FILTER, false);
  public static final FilterPredicates ALL_VARDENHETER =
      new FilterPredicates(
          fact -> fact.getUnderenhet() == null,
          sjukfall -> true,
  ...
  public FilterPredicates createEnhetFilter(HsaIdEnhet... enhetIds) {
    ...
    final Predicate<Fact> factPredicate =
        fact ->
            fact != null
                && (availableEnhets.contains(fact.getVardenhet())
                    || availableEnhets.contains(fact.getUnderenhet()));
    return new FilterPredicates(factPredicate, sjukfall -> true, hashValue, false);
  }

  public List<SjukfallGroup> sjukfallGrupper(
  ...
  public <T> KonDataResponse calculateKonDataResponse(
      Aisle aisle,
      ...
      List<T> groupIds,
      CounterFunction<T> counterFunction) {
    ...
    for (SjukfallGroup sjukfallGroup : sjukfallGrupper(start, periods, periodSize, aisle, filter)) {
      ...
      for (Sjukfall sjukfall : sjukfallGroup.getSjukfall()) {
        final HashMultiset<T> currentCounter =
            Kon.FEMALE.equals(sjukfall.getKon()) ? femaleCounter : maleCounter;
    ...
    return new KonDataResponse(AvailableFilters.getForSjukfall(), groupNames, rows);
  }
  ...
  public <T> KonDataResponse calculateKonDataResponse(
      ...
      int periods,
      int periodSize,
      Function<Sjukfall, Collection<T>> groupsFunction,
      CounterFunction<T> counterFunction) {
    List<KonDataRow> rows = new ArrayList<>();
    final Iterable<SjukfallGroup> sjukfallGroups =
        sjukfallGrupper(start, periods, periodSize, aisle, filter);
    ...
    final List<String> groupNames =
        hashSet.stream().map(String::valueOf).collect(Collectors.toList());
    return new KonDataResponse(AvailableFilters.getForSjukfall(), groupNames, rows);
  }

  // CHECKSTYLE:ON

  public SimpleKonResponse calculateSimpleKonResponse(
      Aisle aisle,
      FilterPredicates filter,
      LocalDate from,
      int periods,
      ...
      List<Integer> groups) {
    return calculateSimpleKonResponse(
        toCount, groups, sjukfallGrupper(from, periods, periodLength, aisle, filter));
  }

  private SimpleKonResponse calculateSimpleKonResponse(
      CounterFunction<Integer> toCount,
      List<Integer> groups,
    ...
    for (SjukfallGroup sjukfallGroup : sjukfallGroups) {
      for (Sjukfall sjukfall : sjukfallGroup.getSjukfall()) {
        HashMultiset<Integer> counter =
            Kon.MALE.equals(sjukfall.getKon()) ? maleCounter : femaleCounter;
    ...
    for (Integer group : groups) {
      rows.add(
          new SimpleKonDataRow(
              String.valueOf(group), femaleCounter.count(group), maleCounter.count(group)));
    }
