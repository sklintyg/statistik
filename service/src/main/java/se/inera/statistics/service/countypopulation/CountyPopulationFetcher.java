...
public class CountyPopulationFetcher {
  ...
  @Autowired
  @Qualifier("restTemplateStat") private RestTemplate rest;
   ...
   * @return An empty optional if no population for the requested yer was found, otherwise the
  ...
  private Map<String, KonField> parseScbResult(String result, int year) {
    ...
    final Map<String, List<ScbRow>> rowsPerCounty =
        Arrays.stream(rows)
            .skip(1)
            .map(this::parseScbRow)
            .collect(Collectors.groupingBy(scbRow -> scbRow.countyId));
    return rowsPerCounty.entrySet().stream()
        .collect(Collectors.toMap(Map.Entry::getKey, e -> toKonField(e.getValue())));
  }
  ...
  private KonField toKonField(List<ScbRow> konValuesForSingleCounty) {
    ...
    if (!valuePerKon.containsKey(Kon.FEMALE) || !valuePerKon.containsKey(Kon.MALE)) {
      final String countyId =
          konValuesForSingleCounty.isEmpty() ? "?" : konValuesForSingleCounty.get(0).countyId;
      throw new ScbPopulationException(
          "Can't find population for both genders on county: " + countyId);
    }
    return new KonField(valuePerKon.get(Kon.FEMALE), valuePerKon.get(Kon.MALE));
  }

  @java.lang.SuppressWarnings(
      "squid:UnusedPrivateMethod") // SONAR reports this method as not used due to
  ...
  private ScbRow parseScbRow(String row) {
    ...
    if (!countyId.matches("^[0-9][0-9]$")) {
      throw new ScbPopulationException(
          "County id should be an integer with two digits: " + countyId);
    }
  ...
  private String requestPopulationFromScb(int year) {
    try {
      ...
      if (response == null) {
        throw new ScbPopulationException(
            "Null returned from SCB for county population for " + year);
      }
      return response;
    ...
    try (BufferedReader in =
        new BufferedReader(
            new InputStreamReader(
                CountyPopulationFetcher.class.getResourceAsStream(path), StandardCharsets.UTF_8))) {
      ...
      throw new ScbPopulationException(
          "Could not read scb county population request resource file", e);
    }
  }
}
