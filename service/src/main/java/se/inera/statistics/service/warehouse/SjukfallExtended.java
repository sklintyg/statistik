...
public class SjukfallExtended {
  ...
  private static final Comparator<Fact> START_DATUM_SORTER =
      (f1, f2) -> {
  ...
  /**
   * Checks if the given Fact is part of this Sjukfall. If so, this sjukfall is updated and return,
   * otherwise a new sjukfall is created.
   *
   * @return join will either return the same, possibly modified (i.e. this), Sjukfall-object, or a
   *     new object
   */
  ...
  public SjukfallExtended extendSjukfallWithPeriods(SjukfallExtended sjukfallWithExtendingPeriods) {
    ...
    for (Fact fact : sjukfallWithExtendingPeriods.facts) {
      sjukfall.sjukskrivningsperiods.add(
          new Sjukskrivningsperiod(fact.getStartdatum(), fact.getSjukskrivningslangd()));
    }
    return sjukfall;
  }
  ...
  public String toString() {
    return "Sjukfall{"
        + "start="
        + WidelineConverter.toDate(start)
        + " ("
        + start
        + ")"
        + ", end="
        + WidelineConverter.toDate(end)
        + " ("
        + end
        + ")"
        + ", realDays="
  ...
  public Set<Lakare> getLakare() {
    return facts.stream()
        .sorted(START_DATUM_SORTER.reversed())
        .map(this::getLakareFromFact)
  ...
  public SjukfallExtended extendWithRealDaysWithinPeriod(SjukfallExtended previous) {
    ...
    for (Sjukskrivningsperiod sjukskrivningsperiod : previous.sjukskrivningsperiods) {
      ...
      if (startIsInPeriod && endIsInPeriod) {
        ...
        sjukfall.sjukskrivningsperiods.add(
            new Sjukskrivningsperiod(startInPeriod, newPeriodLength));
      }
    }
  ...
  private void addAllWithinPeriodInAllIntygUsedInSjukfall(
      SjukfallExtended sjukfallExtended, NavigableSet<Fact> facts) {
    for (Fact fact : facts) {
      if (fact.getStartdatum() > sjukfallExtended.getStart()
          && fact.getSlutdatum() < sjukfallExtended.getEnd()) {
