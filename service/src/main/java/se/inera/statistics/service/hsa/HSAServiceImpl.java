...
public class HSAServiceImpl implements HSAService {
  ...
  @Autowired private HsaStatisticsService service;
  ...
  private HsaInfo getHsaInfoWithoutExceptionHandling(HSAKey key, HsaInfo baseHsaInfo) {
    ...
    if (noEnhetIsDefined(enhet)
      ...
      if (unit != null) {
        // huvudenhet=vårdenhet och enhet kan vara vårdenhet, om det inte finns huvudenhet, annars
        // motsvarar det
  ...
  private boolean enhetHasNoVgAndNoHuvudenhetIsDefined(
      HsaInfoEnhet enhet, HsaInfoEnhet huvudenhet) {
  ...
  private HsaInfoVg getVardgivare(
      HSAKey key, HsaInfo nonNullBase, HsaInfoEnhet enhet, HsaInfoEnhet huvudenhet) {
    ...
    if (vardgivare == null) {
      ...
      if (vgIdFromHsa == null || !vgIdFromHsa.equals(key.getVardgivareId())) {
        LOG.info(
            "VardgivarId mismatch found for enhet: {}. Expected {} from intyg but was {} in HSA",
            key.getEnhetId(),
            key.getVardgivareId(),
            vgIdFromHsa);
      }
  ...
  private HsaInfoPersonal createPersonal(
      GetStatisticsPersonResponseDto personal, GetStatisticsNamesResponseDto names) {
    if (personal == null) {
      return null;
    }
  ...
  private HsaInfoEnhetGeo createGeografiskIndelning(StatisticsHsaUnitDto unit) {
    ...
    final HsaInfoEnhetGeo hsaInfoEnhetGeo =
        new HsaInfoEnhetGeo(
            coordinate,
            location,
            municipalitySectionCode,
            municipalitySectionName,
            municipalityCode,
            countyCode);
