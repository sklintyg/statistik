...
class FactConverter {
  ...
  Fact toFact(WideLine wideline) {
    ...
    int[] lakarbefattnings =
        parseBefattning(wideline.getLakarbefattning(), wideline.getCorrelationId());
    ...
    return new Fact(
        ...
        kon,
        alder,
        extractKapitel(diagnoskapitel),
        extractAvsnitt(diagnosavsnitt),
        extractKategori(diagnoskategori),
        extractKod(diagnoskod, diagnoskategori),
        sjukskrivningsgrad,
        lakarkon,
        lakaralder,
        lakarbefattnings,
  ...
  int[] parseBefattning(String lakarbefattningString, String correlationId) {
    ...
    return Arrays.copyOfRange(
        befattnings,
        0,
        parsedBefattningsAdded); // ArrayUtils.toPrimitive(befattnings.toArray(new Integer[0]));
  }
  ...
  int extractKod(String diagnoskod, String diagnoskategori) {
    ...
    if (kod == null) {
      Icd10.Kategori kategori = icd10.getKategori(diagnoskategori);
      final Optional<Icd10.Kod> unknownKodInKatergori =
          kategori != null ? kategori.getUnknownKod() : Optional.empty();
      return unknownKodInKatergori
          .map(Icd10.Kod::toInt)
