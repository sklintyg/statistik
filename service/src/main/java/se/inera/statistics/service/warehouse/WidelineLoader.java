...
public class WidelineLoader {
  ...
  @Autowired private FactConverter factConverter;

  public List<Aisle> getAilesForVgs(List<HsaIdVardgivare> vgids) {
    LOG.info(
        "Get facts for vgs: "
            + String.join(", ", vgids.stream().map(HsaIdAny::getId).collect(Collectors.toList())));
  ...
  private Fact toFact(ResultSet resultSet) throws SQLException {
    ...
    return new Fact(
        id,
        ConversionHelper.extractLan(lkf),
        ConversionHelper.extractKommun(lkf),
        ConversionHelper.extractForsamling(lkf),
        new HsaIdEnhet(vardenhet),
        new HsaIdEnhet(enhet),
        intyg,
        ConversionHelper.patientIdToInt(patientid),
        startdatum,
        slutdatum,
        kon,
        alder,
        factConverter.extractKapitel(diagnoskapitel),
        factConverter.extractAvsnitt(diagnosavsnitt),
        factConverter.extractKategori(diagnoskategori),
        factConverter.extractKod(diagnoskod, diagnoskategori),
        sjukskrivningsgrad,
        lakarkon,
        lakaralder,
        factConverter.parseBefattning(lakarbefattning, correlationId),
  ...
  private PreparedStatement prepareStatementForVg(
      Connection connection, List<HsaIdVardgivare> vgids) throws SQLException {
    final String vgidsJoined =
        String.join("', '", vgids.stream().map(HsaIdAny::getId).collect(Collectors.toList()));
    String sql =
        "SELECT id, correlationid, lkf, enhet, vardenhet, lakarintyg, patientid, startdatum, slutdatum, kon, alder,"
            + " diagnoskapitel, diagnosavsnitt, diagnoskategori, diagnoskod, sjukskrivningsgrad, lakarkon, lakaralder, "
            + "lakarbefattning, vardgivareid, lakareid, active FROM wideline WHERE active AND vardgivareid IN ('"
            + vgidsJoined
            + "')";
  ...
  List<VgNumber> getAllVgs() {
    ...
    final String sql =
        "SELECT vardgivareid, count(vardgivareid) AS antal FROM intygcommon w1 "
