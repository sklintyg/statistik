...
/**
 * This is a common cache used to get better performance in ST. It is a shared cache to make it
 * easier to maintain the memory footprint.
 */
@Component
public class Cache {
  ...
  @Autowired
  @Qualifier("rediscache") private RedisTemplate<Object, Object> template;
  ...
  public List<HsaIdEnhet> getVgEnhets(
      HsaIdVardgivare vardgivareId, Function<HsaIdVardgivare, List<Enhet>> loader) {
  ...
  public Aisle getAisle(HsaIdVardgivare vardgivarId, Function<HsaIdVardgivare, Aisle> loader) {
    ...
    return lookup(
        AISLE + vardgivarId.getId(),
        () -> {
          return aisleLoader(vardgivarId, loader);
        });
  }

  public Collection<Enhet> getEnhetsWithHsaId(
      final Collection<HsaIdEnhet> enhetIds,
      final Function<Collection<HsaIdEnhet>, Collection<Enhet>> loader) {
    final ValueOperations<Object, Object> ops = template.opsForValue();
    final List<Enhet> cachedEnhets =
        ops
            .multiGet(enhetIds.stream().map(v -> ENHET + v.getId()).collect(Collectors.toList()))
            .stream()
    ...
    final List<HsaIdEnhet> hsaIdsInCache =
        cachedEnhets.stream().map(Enhet::getEnhetId).collect(Collectors.toList());
    hsaIdEnhetsNotInCache.removeAll(hsaIdsInCache);

    final Set<Enhet> enhets =
        loader.apply(hsaIdEnhetsNotInCache).stream()
  ...
  public List<HsaIdEnhet> getEnhetWithVardenhetsId(
      HsaIdEnhet vardenhetId, Function<HsaIdEnhet, List<Enhet>> loader) {
    LOG.info("Getting VardenhetEnhets: {}", vardenhetId);
    return lookup(
        VARDENHETENHET + vardenhetId.getId(), () -> loadEnhetWithVardenhetsId(vardenhetId, loader));
  }

  private List<HsaIdEnhet> loadEnhetWithVardenhetsId(
      HsaIdEnhet vardenhetId, Function<HsaIdEnhet, List<Enhet>> loader) {
  ...
  private List<SjukfallGroup> loadSjukfallgroups(SjukfallGroupCacheKey key) {
    ...
    return Lists.newArrayList(new SjukfallIterator(from, periods, periodSize, aisle, filter));
  }
  ...
  private List<HsaIdEnhet> loadVgEnhets(
      HsaIdVardgivare vardgivareId, Function<HsaIdVardgivare, List<Enhet>> loader) {
