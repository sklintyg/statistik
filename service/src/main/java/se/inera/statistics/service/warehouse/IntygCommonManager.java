...
public class IntygCommonManager {
  ...
  public void persistIfValid(long logId, String intygid, IntygCommon line) {
    ...
    if (errors.isEmpty()) {
      ...
      StringBuilder errorBuilder =
          new StringBuilder("Faulty intyg logid ")
              .append(logId)
              .append(" id ")
              .append(intygid)
              .append(" error count ")
  ...
  private boolean revokeIntygsId(IntygCommon line) {
    return EventType.REVOKED.equals(line.getEventType())
        || isIntygsIdAlreadyRevoked(line.getIntygid());
  }
  ...
  private void inactivateIntyg(String intygid) {
    final Query query =
        manager.createQuery("UPDATE IntygCommon SET active = false WHERE intygid = :intygid");
    query.setParameter("intygid", intygid);
    query.executeUpdate();
  }
  ...
  public SimpleKonResponse getIntyg(HsaIdVardgivare vardgivarId, IntygCommonFilter intygFilter) {
    final Function<IntygCommonGroup, String> rowNameFunction =
        intygCommonGroup -> ReportUtil.toDiagramPeriod(intygCommonGroup.getRange().getFrom());
    return getIntygCommonMaleFemale(vardgivarId, intygFilter, rowNameFunction, false);
  }

  public SimpleKonResponse getIntygTvarsnitt(
      HsaIdVardgivare vardgivarId, IntygCommonFilter intygFilter) {
  ...
  public KonDataResponse getIntygPerTypeTidsserie(
      HsaIdVardgivare vardgivarId, IntygCommonFilter intygFilter) {
    return getIntygPerType(vardgivarId, intygFilter, false);
  }

  public KonDataResponse getIntygPerTypeTidsserieAggregated(
      KonDataResponse resultToAggregateIn,
      HsaIdVardgivare vardgivarId,
      IntygCommonFilter intygFilter,
      int cutoff) {
    final KonDataResponse messagesTvarsnittPerAmne =
        getIntygPerType(vardgivarId, intygFilter, false);
    final KonDataResponse resultToAggregate =
        resultToAggregateIn != null
            ? resultToAggregateIn
            : ResponseUtil.createEmptyKonDataResponse(messagesTvarsnittPerAmne);
    ...
    List<KonDataRow> list =
        ResponseUtil.getKonDataRows(
            intygFilter.getRange().getNumberOfMonths(), rowsNew, rowsOld, cutoff);
    return new KonDataResponse(
        AvailableFilters.getForIntyg(), messagesTvarsnittPerAmne.getGroups(), list);
  }

  private KonDataResponse getIntygPerType(
      HsaIdVardgivare vardgivarId, IntygCommonFilter intygFilter, boolean isTvarsnitt) {
    ...
    IntygCommonFilter newIntygFilter =
        new IntygCommonFilter(
            intygFilter.getRange(),
            intygFilter.getEnheter(),
            intygFilter.getDiagnoser(),
            intygFilter.getAldersgrupp(),
            null);
    ...
    final CounterFunctionIntyg<Integer> counterFunction =
        (intyg, counter) -> {
          final IntygType intygType = intyg.getIntygtyp();
          final int id =
              IntygType.FK7263.equals(intygType) ? IntygType.LISJP.ordinal() : intygType.ordinal();
          counter.add(id);
        };
    return calculateKonDataResponse(
        vardgivarId, newIntygFilter, isTvarsnitt, names, ids, counterFunction);
  }
  ...
  private <T> KonDataResponse calculateKonDataResponse(
      HsaIdVardgivare vardgivarId,
      IntygCommonFilter intygFilter,
      boolean isTvarsnitt,
      List<String> groupNames,
      List<T> groupIds,
      CounterFunctionIntyg<T> counterFunction) {
    List<KonDataRow> rows = new ArrayList<>();
    final List<IntygCommonGroup> intygCommonGroups =
        getIntygCommonGroups(vardgivarId, intygFilter, isTvarsnitt, null);
    for (IntygCommonGroup intygCommonGroup : intygCommonGroups) {
      ...
      rows.add(
          new KonDataRow(ReportUtil.toDiagramPeriod(intygCommonGroup.getRange().getFrom()), list));
    }

    return new KonDataResponse(AvailableFilters.getForIntyg(), groupNames, rows);
  }

  private SimpleKonResponse getIntygCommonMaleFemale(
      HsaIdVardgivare vardgivarId,
      IntygCommonFilter intygFilter,
      Function<IntygCommonGroup, String> rowNameFunction,
      boolean isTvarsnitt) {
    ...
    final List<IntygType> intygTypes =
        Arrays.asList(
            ...
            IntygType.LUAE_NA,
            IntygType.LUAE_FS);
    for (IntygCommonGroup intygCommonGroup :
        getIntygCommonGroups(vardgivarId, intygFilter, isTvarsnitt, intygTypes)) {
  ...
  private List<IntygCommonGroup> getIntygCommonGroups(
      HsaIdVardgivare vardgivarId,
      IntygCommonFilter intygFilter,
      boolean isTvarsnitt,
    ...
    IntygCommonGroup intygCommonGroup =
        getIntygCommonGroup(range, vardgivarId, intygFilter, intygTypes);
    ...
    for (int i = 0; i < periods; i++) {
      ...
      final IntygCommonGroup periodGroup =
          new IntygCommonGroup(
              newRange,
              intygCommonGroup.getIntyg().stream()
  ...
  private IntygCommonGroup getIntygCommonGroup(
      Range range,
      HsaIdVardgivare vardgivarId,
      IntygCommonFilter intygFilter,
      List<IntygType> intygTypes) {
    ...
    List<IntygType> intygTypeFilter =
        intygFilter.getIntygstyper() != null && !intygFilter.getIntygstyper().isEmpty()
            ? intygFilter.getIntygstyper().stream()
                .map(IntygType::getByName)
                .filter(Optional::isPresent)
                .map(Optional::get)
                .flatMap(intygType -> intygType.getUnmappedTypes().stream())
                .collect(Collectors.toList())
            : allIntygTypes;
    final List<IntygType> intygTypesToQuery =
        (intygTypes != null ? intygTypes : allIntygTypes)
            .stream().filter(intygTypeFilter::contains).collect(Collectors.toList());
    ...
    final Collection<HsaIdEnhet> enheter =
        EnhetUtil.getAllEnheterInVardenheter(intygFilter.getEnheter(), warehouse);
    if (enheter != null) {
    ...
    if (enheter != null) {
      final List<String> enhetIds =
          enheter.stream().map(HsaIdAny::getId).collect(Collectors.toList());
    ...
    final boolean isAgeFilterActive =
        aldersgrupp != null
            && !aldersgrupp.isEmpty()
            && aldersgrupp.size() != AgeGroup.values().length;
  ...
  public static List<IntygCommon> applyDxFilter(
      Collection<String> dxFilter, List<IntygCommon> dtos, Icd10 icd10) {
  ...
  public static List<IntygCommon> applyAgeFilter(
      List<IntygCommon> resultList, Collection<String> aldersgrupp) {
    final List<AgeGroup> ageGroupFilters =
        aldersgrupp.stream()
    ...
    return resultList.stream()
        .filter(
            intygCommon -> {
              ...
              final int alder =
                  ConversionHelper.extractAlder(patientid, intygCommon.getSigneringsdatum());
  ...
  private boolean isIntygSentToFk(String correlationId) {
    ...
    final CriteriaQuery<IntygSentEvent> query =
        q.select(from).where(cb.equal(from.get("correlationId"), correlationId));
  ...
  public Collection<IntygType> getAllExistingIntygTypes() {
    final TypedQuery<IntygType> q =
        manager.createQuery("SELECT DISTINCT intygtyp FROM IntygCommon", IntygType.class);
