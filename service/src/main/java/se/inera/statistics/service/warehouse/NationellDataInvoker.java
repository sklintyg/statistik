...
public class NationellDataInvoker {
  ...
  public NationellDataInfo getAll() {
    ...
    final NationellDataHolder data =
        new NationellDataHolder(); // Hold data between calculations per vg
  ...
  private void calculateMsgPerAmne(HsaIdVardgivare vardgivareId, NationellDataInfo result) {
    ...
    final MessagesFilter messagesFilter =
        new MessagesFilter(vardgivareId, range.getFrom(), range.getTo(), null, null, null, null);
    final KonDataResponse meddelandenPerAmne = result.getMeddelandenPerAmneResult();
    final KonDataResponse resp =
        messagesQuery.getMeddelandenPerAmneAggregated(meddelandenPerAmne, messagesFilter, cutoff);
  ...
  private void calulateAndelIntyg(HsaIdVardgivare vardgivareId, NationellDataInfo result) {
    ...
    final MessagesFilter messagesFilter =
        new MessagesFilter(vardgivareId, from, to, null, null, null, null);
    final KonDataResponse konDataResponse = result.getAndelKompletteringarResult();
    final KonDataResponse response =
        messagesQuery.getAndelKompletteringarAggregated(konDataResponse, messagesFilter, cutoff);
    result.setAndelKompletteringarResult(response);
  }

  private void calulateKompletteringarPerFraga(
      HsaIdVardgivare vardgivareId, NationellDataInfo result) {
    ...
    final SimpleKonResponse response =
        messagesQuery.getKompletteringarPerFragaTvarsnittAggregated(
  ...
  private void calculateIntygPerVg(HsaIdVardgivare vardgivareId, NationellDataInfo result) {
    ...
    final IntygCommonFilter intygCommonFilter =
        new IntygCommonFilter(range, null, null, null, null);
    final KonDataResponse konDataResponse = result.getIntygPerTypResult();
    final KonDataResponse response =
        intygCommonManager.getIntygPerTypeTidsserieAggregated(
            konDataResponse, vardgivareId, intygCommonFilter, cutoff);
    result.setIntygPerTypResult(response);
  }

  private void calculatePerAisle(
      NationellData nationellData,
      NationellDataInfo result,
      NationellDataHolder data,
      Aisle aisle) {
    final Range antalRange = result.getAntalIntygRange();
    nationellData.updateAntalIntygResult(
        aisle, antalRange.getFrom(), antalRange.getNumberOfMonths(), 1, data.getAntalIntygResult());

    data.setDiagnosgrupperResult(
        nationellData.getDiagnosgrupper(
            aisle, result.getDiagnosgrupperRange(), data.getDiagnosgrupperResult()));

    for (Icd10.Kapitel kapitel : icd10.getKapitel(false)) {
      nationellData.getDiagnosavsnitt(
          aisle, result.getDiagnosavsnittRange(), kapitel, data.getDiagnosavsnittResult());
    }

    data.setAldersgrupperResult(
        nationellData.getAldersgrupper(
            aisle,
            result.getAldersgrupperRange(),
            data.getAldersgrupperResult(),
            AldersgruppQuery.RANGES));

    data.setSjukskrivningsgradResult(
        nationellData.getSjukskrivningsgrad(
            aisle, result.getSjukskrivningsgradRange(), data.getSjukskrivningsgradResult()));

    data.setSjukfallslangdResult(
        nationellData.getSjukfallslangd(
            aisle, result.getSjukfallslangdRange(), data.getSjukfallslangdResult()));

    nationellData.addSjukfallPerLanToResult(result.getLanRange(), data.getLanResult(), aisle);

    data.setIntygPerSjukfallResult(
        nationellData.getIntygPerSjukfall(
            aisle, result.getIntygPerSjukfallRange(), data.getIntygPerSjukfallResult()));
    ...
    data.setOverviewForandringCurrentResult(
        nationellData.getAntalIntygOverviewResult(
            aisle, overviewRange, data.getOverviewForandringCurrentResult()));
    data.setOverviewForandringPreviousResult(
        nationellData.getAntalIntygOverviewResult(
            aisle, previousOverviewRange, data.getOverviewForandringPreviousResult()));
    data.setOverviewDiagnosgrupperCurrentResult(
        nationellData.getDiagnosgrupperOverview(
            aisle, overviewRange, data.getOverviewDiagnosgrupperCurrentResult()));
    data.setOverviewDiagnosgrupperPreviousResult(
        nationellData.getDiagnosgrupperOverview(
            aisle, previousOverviewRange, data.getOverviewDiagnosgrupperPreviousResult()));

    data.setOverviewPreviousAldersgrupperResult(
        nationellData.getAldersgrupper(
            aisle,
            previousOverviewRange,
            data.getOverviewPreviousAldersgrupperResult(),
            AldersgruppQuery.OVERVIEW_RANGES));
    data.setOverviewCurrentAldersgrupperResult(
        nationellData.getAldersgrupper(
            aisle,
            overviewRange,
            data.getOverviewCurrentAldersgrupperResult(),
            AldersgruppQuery.OVERVIEW_RANGES));

    data.setOverviewSjukskrivningsgradCurrentResult(
        nationellData.getSjukskrivningsgradOverview(
            aisle, overviewRange, data.getOverviewSjukskrivningsgradCurrentResult()));
    data.setOverviewSjukskrivningsgradPreviousResult(
        nationellData.getSjukskrivningsgradOverview(
            aisle, previousOverviewRange, data.getOverviewSjukskrivningsgradPreviousResult()));

    data.setOverviewSjukfallslangdPreviousResult(
        nationellData.getSjukfallslangd(
            aisle, previousOverviewRange, data.getOverviewSjukfallslangdPreviousResult()));
    data.setOverviewSjukfallslangdCurrentResult(
        nationellData.getSjukfallslangd(
            aisle, overviewRange, data.getOverviewSjukfallslangdCurrentResult()));

    data.setOverviewLangaSjukfallDiffPreviousResult(
        nationellData.getLangaSjukfall(
            aisle, previousOverviewRange, data.getOverviewLangaSjukfallDiffPreviousResult()));
    data.setOverviewLangaSjukfallDiffCurrentResult(
        nationellData.getLangaSjukfall(
            aisle, overviewRange, data.getOverviewLangaSjukfallDiffCurrentResult()));

    nationellData.addSjukfallPerLanToResult(
        previousOverviewRange, data.getOverviewLanPreviousResult(), aisle);
    nationellData.addSjukfallPerLanToResult(
        overviewRange, data.getOverviewLanCurrentResult(), aisle);
  }
  ...
  private void populateRanges(NationellDataInfo result) {
    ...
    result.setAldersgrupperRange(
        Range.createForLastMonthsExcludingCurrent(yearRange.getNumberOfMonths(), clock));
    result.setSjukskrivningsgradRange(longRange);
    result.setSjukfallslangdRange(
        Range.createForLastMonthsExcludingCurrent(yearRange.getNumberOfMonths(), clock));
    ...
    result.setIntygPerSjukfallRange(
        Range.createForLastMonthsExcludingCurrent(yearRange.getNumberOfMonths(), clock));
  ...
  private NationellDataInfo populateResults(NationellDataInfo result, NationellDataHolder data) {
    ...
    result.setAntalIntygResult(
        new SimpleKonResponse(AvailableFilters.getForNationell(), data.getAntalIntygResult()));

    if (data.getDiagnosgrupperResult() == null) {
      data.setDiagnosgrupperResult(
          new DiagnosgruppResponse(
              AvailableFilters.getForNationell(), new ArrayList<>(), new ArrayList<>()));
    }
    result.setDiagnosgrupperResult(data.getDiagnosgrupperResult());

    for (Icd10.Kapitel kapitel : icd10.getKapitel(false)) {
      if (!data.getDiagnosavsnittResult().containsKey(kapitel)) {
        DiagnosgruppResponse response =
            new DiagnosgruppResponse(
                AvailableFilters.getForNationell(), new ArrayList<>(), new ArrayList<>());
    ...
    if (data.getAldersgrupperResult() == null) {
      data.setAldersgrupperResult(
          new SimpleKonResponse(AvailableFilters.getForNationell(), new ArrayList<>()));
    }
    ...
    result.setLanResult(
        new SimpleKonResponse(AvailableFilters.getForNationell(), data.getLanResult()));

    result.setOverviewGenderResult(
        new SimpleKonResponse(
            AvailableFilters.getForNationell(), data.getOverviewAntalIntygResult()));
    ...
    result.setOverviewSjukskrivningsgraderPrevious(
        data.getOverviewSjukskrivningsgradPreviousResult());
    result.setOverviewSjukskrivningsgraderCurrent(
        data.getOverviewSjukskrivningsgradCurrentResult());
    result.setOverviewSjukskrivningslangdPreviousResult(
        data.getOverviewSjukfallslangdPreviousResult());
    result.setOverviewSjukskrivningslangdCurrentResult(
        data.getOverviewSjukfallslangdCurrentResult());
    result.setOverviewLangaSjukfallResult(
        toLangaSjukfall(data.getOverviewSjukfallslangdCurrentResult()));
    result.setOverviewLangaSjukfallDiffCurrentResult(
        data.getOverviewLangaSjukfallDiffCurrentResult());
    result.setOverviewLangaSjukfallDiffPreviousResult(
        data.getOverviewLangaSjukfallDiffPreviousResult());
    result.setOverviewLanPreviousResult(
        new SimpleKonResponse(
            AvailableFilters.getForNationell(), data.getOverviewLanPreviousResult()));
    result.setOverviewLanCurrentResult(
        new SimpleKonResponse(
            AvailableFilters.getForNationell(), data.getOverviewLanCurrentResult()));
  ...
  private Integer toLangaSjukfall(SimpleKonResponse resp) {
    ...
    return rows.stream()
        ...
        .filter(
            r ->
                SjukfallsLangdGroup.getByName(r.getName())
                    .map(SjukfallsLangdGroup::isLongSjukfallInOverview)
                    .orElse(false))
