...
class NationellData {
  ...
  SimpleKonDataRow getAntalIntygOverviewResult(
      Aisle aisle, Range range, SimpleKonDataRow antalIntygResult) {
  ...
  void updateAntalIntygResult(
      Aisle aisle,
      LocalDate start,
      int perioder,
      int periodlangd,
      List<SimpleKonDataRow> antalIntygResult) {
    int index = 0;
    for (SjukfallGroup sjukfallGroup :
        sjukfallUtil.sjukfallGrupper(
            start, perioder, periodlangd, aisle, SjukfallUtil.ALL_ENHETER)) {
      ...
      if (index >= antalIntygResult.size()) {
        antalIntygResult.add(
            new SimpleKonDataRow(
                displayDate, filterCutoff(female, cutoff), filterCutoff(male, cutoff)));
      } else {
        SimpleKonDataRow previous = antalIntygResult.get(index);
        antalIntygResult.set(
            index,
            new SimpleKonDataRow(
                previous.getName(),
  ...
  SimpleKonResponse getAldersgrupper(
      Aisle aisle, Range range, SimpleKonResponse aldersgrupperResult, Ranges ranges) {
    SimpleKonResponse aldersgrupper =
    ...
    while (rowsNew.hasNext() && rowsOld.hasNext()) {
      ...
      list.add(
          new SimpleKonDataRow(
              a.getName(),
              filterCutoff(a.getFemale(), cutoff) + b.getFemale(),
              filterCutoff(a.getMale(), cutoff) + b.getMale()));
    }
    return new SimpleKonResponse(AvailableFilters.getForNationell(), list);
  }

  KonDataResponse getSjukskrivningsgrad(
      Aisle aisle, Range range, KonDataResponse sjukskrivningsgradResult) {
    return getSjukskrivningsgrad(
        aisle, range.getFrom(), range.getNumberOfMonths(), 1, false, sjukskrivningsgradResult);
  }

  KonDataResponse getSjukskrivningsgradOverview(
      Aisle aisle, Range range, KonDataResponse sjukskrivningsgradResult) {
    return getSjukskrivningsgrad(
        aisle, range.getFrom(), 1, range.getNumberOfMonths(), true, sjukskrivningsgradResult);
  }

  private KonDataResponse getSjukskrivningsgrad(
      Aisle aisle,
      LocalDate start,
      int perioder,
      int periodlangd,
      boolean all,
      KonDataResponse sjukskrivningsgradResult) {
    KonDataResponse grader =
        SjukskrivningsgradQuery.getSjukskrivningsgrad(
            aisle, SjukfallUtil.ALL_ENHETER, start, perioder, periodlangd, sjukfallUtil, all);
    ...
    return new KonDataResponse(
        AvailableFilters.getForNationell(), sjukskrivningsgradResult.getGroups(), list);
  }

  SimpleKonResponse getSjukfallslangd(
      Aisle aisle, Range range, SimpleKonResponse sjukfallslangdResult) {
    SimpleKonResponse langder =
  ...
  DiagnosgruppResponse getDiagnosgrupperOverview(
      Aisle aisle, Range range, DiagnosgruppResponse diagnosgrupperResult) {
    return getDiagnosgrupper(
        aisle, range.getFrom(), 1, range.getNumberOfMonths(), true, diagnosgrupperResult);
  }

  DiagnosgruppResponse getDiagnosgrupper(
      Aisle aisle, Range range, DiagnosgruppResponse diagnosgrupperResult) {
    return getDiagnosgrupper(
        aisle, range.getFrom(), range.getNumberOfMonths(), 1, false, diagnosgrupperResult);
  }

  private DiagnosgruppResponse getDiagnosgrupper(
      Aisle aisle,
      LocalDate start,
      int perioder,
      int periodlangd,
      boolean all,
      DiagnosgruppResponse diagnosgrupperResult) {
    final FilterPredicates filter = SjukfallUtil.ALL_ENHETER;
    DiagnosgruppResponse diagnosgrupper =
        query.getDiagnosgrupper(aisle, filter, start, perioder, periodlangd, all);
    ...
    while (rowsNew.hasNext() && rowsOld.hasNext()) {
      ...
      for (int i = 0; i < maxDataSize; i++) {
        c.add(
            new KonField(safeSumForIndex(a, b, i, Kon.FEMALE), safeSumForIndex(a, b, i, Kon.MALE)));
      }
      list.add(new KonDataRow(a.getName(), c));
    }
    final List<? extends Icd> icdTyps =
        diagnosgrupperResult.getIcdTyps().size() < diagnosgrupper.getIcdTyps().size()
            ? diagnosgrupper.getIcdTyps()
            : diagnosgrupperResult.getIcdTyps();
  ...
  private int safeSumForIndex(KonDataRow newRow, KonDataRow existingRow, int index, Kon kon) {
    return filterCutoff(getValueSafe(newRow, index, kon), cutoff)
        + getValueSafe(existingRow, index, kon);
  }

  private int getValueSafe(KonDataRow a, int index, Kon kon) {
    if (a == null
        || a.getData() == null
        || a.getData().size() <= index
        || a.getData().get(index) == null) {
      return 0;
    }
    return a.getData().get(index).getValue(kon);
  }

  void getDiagnosavsnitt(
      Aisle aisle,
      Range range,
      Icd10.Kapitel kapitel,
      Map<Icd10.Kapitel, DiagnosgruppResponse> diagnosavsnitts) {
    DiagnosgruppResponse diagnosgrupper =
        query.getDiagnosavsnitts(
            aisle,
            SjukfallUtil.ALL_ENHETER,
            range.getFrom(),
            range.getNumberOfMonths(),
    ...
    while (rowsNew.hasNext() && rowsOld.hasNext()) {
      ...
      for (int i = 0; i < a.getData().size(); i++) {
        final int female =
            filterCutoff(a.getData().get(i).getFemale(), cutoff) + b.getData().get(i).getFemale();
        final int male =
    ...
    DiagnosgruppResponse response =
        new DiagnosgruppResponse(
            AvailableFilters.getForNationell(), diagnosavsnitts.get(kapitel).getIcdTyps(), list);
  ...
  void addSjukfallPerLanToResult(Range range, ArrayList<SimpleKonDataRow> result, Aisle aisle) {
    ...
    for (SjukfallGroup sjukfallGroup :
        sjukfallUtil.sjukfallGrupper(
            range.getFrom(), 1, range.getNumberOfMonths(), aisle, filter)) {
  ...
  SimpleKonResponse getIntygPerSjukfall(
      Aisle aisle, Range range, SimpleKonResponse intygPerSjukfallResult) {
    SimpleKonResponse intygPerSjukfall =
        IntygPerSjukfallQuery.getIntygPerSjukfallTvarsnitt(
            aisle,
            SjukfallUtil.ALL_ENHETER,
            range.getFrom(),
            1,
