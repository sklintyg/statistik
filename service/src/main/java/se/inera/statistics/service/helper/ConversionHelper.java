...
public final class ConversionHelper {
  ...
  public static String getUnifiedPersonId(String personIdRaw1) {
    ...
    String safePersonId =
        personIdRaw1 != null ? personIdRaw1.replaceAll("(^\\h*)|(\\h*$)", "") : "";
  ...
  /**
   * Same as extractAlder() method but will return NO_AGE instead of throwing exception when
   * personId can not be parsed.
   */
  ...
  public static int extractAlder(String personId, LocalDate start) {
    ...
    try {
      ...
      int month =
          Integer.parseInt(dateString.substring(MONTH_PART_OF_DATE_PART, DAY_PART_OF_DATE_PART));

      if (day > SAMORDNINGSNUMMER_DAY_CONSTANT) {
        dateString =
            dateString.substring(0, MONTH_PART_OF_DATE_PART)
                + MONTHDAY_FORMATTER.format(
                    MonthDay.of(month, day - SAMORDNINGSNUMMER_DAY_CONSTANT));
      }
  ...
  /**
   * This is an edited copy of the built in Integer.parseInt() method that will return null instead
   * of throwing an exception when parsing failed. This gives us better performance.
   */
