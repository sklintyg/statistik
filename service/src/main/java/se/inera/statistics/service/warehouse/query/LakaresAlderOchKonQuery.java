...
public final class LakaresAlderOchKonQuery {
  ...
  private static final Ranges RANGES_LAKARES_ALDER_OCH_KON =
      new Ranges(
          range("under 30 år", 30),
          range("30-39 år", 40),
          range("40-49 år", 50),
          range("50-59 år", 60),
          range("över 59 år", Integer.MAX_VALUE));
  ...
  public SimpleKonResponse getSjukfallPerLakaresAlderOchKon(
    ...
    final KonDataResponse konDataResponse =
        getSjukfallPerLakaresAlderOchKonCommon(
            aisle, filter, start, periods, periodLength, getLakaresFunc);
    return SimpleKonResponse.create(konDataResponse);
  }

  public KonDataResponse getSjukfallPerLakaresAlderOchKonSomTidsserie(
      Aisle aisle, FilterPredicates filter, LocalDate start, int periods, int periodLength) {
    final Function<Sjukfall, Collection<Lakare>> getLakaresFunc =
        sjukfall1 -> Collections.singleton(sjukfall1.getLastLakare());
    return getSjukfallPerLakaresAlderOchKonCommon(
        aisle, filter, start, periods, periodLength, getLakaresFunc);
  }

  private KonDataResponse getSjukfallPerLakaresAlderOchKonCommon(
      Aisle aisle,
    ...
    final CounterFunction<String> counterFunction =
        new CounterFunction<String>() {
          @Override
    ...
    final KonDataResponse response =
        sjukfallUtil.calculateKonDataResponse(
            aisle, filter, start, periods, periodLength, names, names, counterFunction);
    final List<String> groupsThatShouldAlwaysBeRetained =
        getGroupsThatShouldAlwaysBeRetained(rangesList);
