...
public class CountyPopulationManagerImpl implements CountyPopulationManagerForTest {
  ...
  @Autowired private CountyPopulationFetcher countyPopulationFetcher;
  ...
  private Optional<CountyPopulation> getCountyPopulation(LocalDate date) {
    final Optional<CountyPopulationRow> preFetchedCountyPopulation =
        getPreFetchedCountyPopulationForDate(date);
    if (preFetchedCountyPopulation.isPresent()) {
      return Optional.of(parsePopulationRow(preFetchedCountyPopulation.get()));
    }
    LOG.info(
        "County population for requested date was not found in local db. Fetching from SCB. Date: "
            + date);
    Optional<CountyPopulation> countyPopulation =
        countyPopulationFetcher.getPopulationFor(date.getYear());
    if (countyPopulation.isPresent()) {
      insertCountyPopulation(countyPopulation.get());
      return countyPopulation;
    }
    LOG.warn(
        "County population for requested date could not be fetched from SCB. Using the last known population.");
    final Optional<CountyPopulationRow> latestPreFetchedCountyPopulation =
        getLatestPreFetchedCountyPopulation();
    if (latestPreFetchedCountyPopulation.isPresent()) {
      return Optional.of(parsePopulationRow(latestPreFetchedCountyPopulation.get()));
    }
    LOG.error(
        "County population could not be fetched from either SCB or local db. Fix connection or populate db manually");
  ...
  private CountyPopulation parsePopulationRow(CountyPopulationRow populationRow) {
    try {
      final MapType mapType =
          mapper.getTypeFactory().constructMapType(HashMap.class, String.class, KonField.class);
      final Map<String, KonField> populationData =
          mapper.readValue(new StringReader(populationRow.getData()), mapType);
  ...
  private Optional<CountyPopulationRow> getPreFetchedCountyPopulationForDate(LocalDate date) {
    return getPreFetchedCountyPopulation(
        date, "SELECT r FROM CountyPopulationRow r WHERE r.date = :fromDate");
  }

  private Optional<CountyPopulationRow> getLatestPreFetchedCountyPopulation() {
    return getPreFetchedCountyPopulation(
        LocalDate.now(clock), "SELECT r FROM CountyPopulationRow r ORDER BY r.date DESC");
  }
  ...
  private void insertCountyPopulation(CountyPopulation countyPopulation) {
    insertCountyPopulation(
        countyPopulation.getPopulationPerCountyCode(), countyPopulation.getDate());
  }
