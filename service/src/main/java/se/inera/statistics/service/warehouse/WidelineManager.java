...
public class WidelineManager {
  ...
  private boolean revokeCorrelationId(WideLine line) {
    return EventType.REVOKED.equals(line.getIntygTyp())
        || isCorrelationIdAlreadyRevoked(line.getCorrelationId());
  }
  ...
  private boolean isCorrelationIdAlreadyRevoked(String correlationId) {
    ...
    final Query query =
        manager.createQuery(
            "SELECT intygTyp, correlationId FROM WideLine "
                + "WHERE intygTyp = "
                + revokedOrdinal
                + " AND correlationId = :corrId");
  ...
  @Transactional
  public int count() {
    return ((Long) manager.createQuery("SELECT COUNT (wl) FROM WideLine wl").getSingleResult())
        .intValue();
  }

  private void inactivateIntyg(String correlationId) {
    final Query query =
        manager.createQuery("UPDATE WideLine SET active = false WHERE correlationId = :corrId");
    query.setParameter("corrId", correlationId);
    query.executeUpdate();
  }
}
