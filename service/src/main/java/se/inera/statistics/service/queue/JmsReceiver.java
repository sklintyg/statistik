...
public class JmsReceiver {
  ...
  private void handleIntyg(Message rawMessage, String doc, String typeName, long timestamp)
      throws JMSException {
    String certificateId = rawMessage.getStringProperty(CERTIFICATE_ID);
    String certificateType = rawMessage.getStringProperty(CERTIFICATE_TYPE);
    if (certificateType != null
        && IntygType.getByItIntygType(certificateType).equals(IntygType.UNKNOWN)) {
      LOG.warn(
          "Received intyg '{}' was discarded since type '{}' is unknown",
          certificateId,
          certificateType);
    } else {
  ...
  private void handleMessage(Message rawMessage, String doc, String typeName, long timestamp)
      throws JMSException {
