...
public class UpdateEnhetNamnFromHsaFileServiceTest {
  ...
  public void testMain() throws Exception {
    ...
    transactionTemplate.execute(new TransactionCallbackWithoutResult() {
      @Override
      protected void doInTransactionWithoutResult(TransactionStatus transactionStatus) {
        final Object[] result1 = (Object[]) manager.createNativeQuery("SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId1 + "'")
            .getSingleResult();
        assertEquals("Hjortsberg", result1[2]);
        final Object[] result2 = (Object[]) manager.createNativeQuery("SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId2 + "'")
            .getSingleResult();
        assertEquals(namn2, result2[2]);
        final Object[] result3 = (Object[]) manager.createNativeQuery("SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId3 + "'")
            .getSingleResult();
