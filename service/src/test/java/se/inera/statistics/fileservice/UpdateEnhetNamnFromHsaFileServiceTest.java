...
public class UpdateEnhetNamnFromHsaFileServiceTest {
  ...
  @Autowired private UpdateEnhetNamnFromHsaFileService updateEnhetNamnFromHsaFileService;
    ...
    transactionTemplate.execute(
        new TransactionCallbackWithoutResult() {
          ...
          private void insertEnhet(final String hsaId, final String namn) {
            manager
                .createNativeQuery(
                    "INSERT INTO enhet (id, enhetId, namn, vardgivareId, lansId, kommunId, verksamhetsTyper) VALUES "
                        + "("
                        + enhetId++
                        + ", '"
                        + hsaId
    ...
    transactionTemplate.execute(
        new TransactionCallbackWithoutResult() {
          ...
          protected void doInTransactionWithoutResult(TransactionStatus status) {
            try {
              updateEnhetNamnFromHsaFileService.doUpdateEnhetnamesFromHsaFileServiceInputstream(
                  hsaUnitFile.openStream());
            } catch (IOException e) {
              fail(e.getMessage());
            }
          }
          ...
          protected void doInTransactionWithoutResult(TransactionStatus transactionStatus) {
            final Object[] result1 =
                (Object[])
                    manager
                        .createNativeQuery(
                            "SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId1 + "'")
                        .getSingleResult();
            ...
            final Object[] result2 =
                (Object[])
                    manager
                        .createNativeQuery(
                            "SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId2 + "'")
                        .getSingleResult();
            ...
            final Object[] result3 =
                (Object[])
                    manager
                        .createNativeQuery(
                            "SELECT id, enhetId, namn FROM enhet WHERE enhetId='" + hsaId3 + "'")
                        .getSingleResult();
  ...
  /**
   * This test will do an end to end test of the fileservice. A webserver will be started locally
   * which makes it possible to download a local fileservice zip file. That file will be downloaded
  ...
  public void e2etest() throws Exception {
    ...
    ReflectionTestUtils.setField(
        updateEnhetNamnFromHsaFileService,
        "hsaunitsUrl",
        "http://localhost:" + port + "/hsazip-fileservice-intyg-p.zip");
    ReflectionTestUtils.setField(
        updateEnhetNamnFromHsaFileService,
        "hsaCertificateFile",
        cl.getResource("certificate.p12").getFile());
    ReflectionTestUtils.setField(
        updateEnhetNamnFromHsaFileService, "hsaCertificatePassword", "statistik");
    ReflectionTestUtils.setField(
        updateEnhetNamnFromHsaFileService,
        "hsaTruststoreFile",
        cl.getResource("test.statistik.jks").getFile());
    ReflectionTestUtils.setField(
        updateEnhetNamnFromHsaFileService, "hsaTruststorePassword", "statistik");
