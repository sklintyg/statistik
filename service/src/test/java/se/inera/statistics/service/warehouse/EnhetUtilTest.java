...
public class EnhetUtilTest {
  ...
  public void testGetAllEnheterInVardenheter() {
    ...
    manager.persist(
        new Enhet(new HsaIdVardgivare("vg1"), new HsaIdEnhet("e3"), "3", "", "", "", "e1"));
    manager.persist(
        new Enhet(new HsaIdVardgivare("vg1"), new HsaIdEnhet("e4"), "4", "", "", "", null));
    manager.persist(
        new Enhet(new HsaIdVardgivare("vg1"), new HsaIdEnhet("e5"), "5", "", "", "", "e4"));
    ...
    // When
    final Collection<HsaIdEnhet> enheterResult1 =
        EnhetUtil.getAllEnheterInVardenheter(enheter1, warehouse);

    final Collection<HsaIdEnhet> enheterResult2 =
        EnhetUtil.getAllEnheterInVardenheter(enheter2, warehouse);

    // Then
    final List<String> enhetIds1 =
        enheterResult1.stream().map(HsaIdEnhet::getId).collect(Collectors.toList());

    final List<String> enhetIds2 =
        enheterResult2.stream().map(HsaIdEnhet::getId).collect(Collectors.toList());
