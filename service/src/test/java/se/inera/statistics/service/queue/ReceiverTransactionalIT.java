...
public class ReceiverTransactionalIT {
  ...
  @Autowired private se.inera.statistics.testsupport.QueueSender queueSender;
  ...
  @Test
  public void unprocessable_document_should_end_up_on_DLQ_because_we_use_transactions()
      throws IOException {
    ...
    int i = 0;
    for (TestIntyg intyg :
        getIntygWithHelsjukAndSingelmanadAndSingelDiagnos(getPerson(PERSON_K1950))) {
    ...
    assertEquals(
        "Verify that unprocessable messages end up on DLQ", 1, getMessagesInQueue(ACTIVEMQ_DLQ));
  }
