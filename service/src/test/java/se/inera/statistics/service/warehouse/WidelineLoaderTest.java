...
public class WidelineLoaderTest {
  ...
  @Autowired WidelineLoader widelineLoader;

  @Autowired WidelineManager widelineManager;

  @Autowired IntygCommonManager intygCommonManager;
  ...
  public void loadPopulatesOnlyWithCreatedLines() {
    ...
    final List<Fact> factsForVg =
        widelineLoader.getAilesForVgs(Collections.singletonList(VG1)).get(0).getLines();
  ...
  private void insertLine(EventType event, String correlationId, HsaIdVardgivare vg) {
    ...
    final IntygCommon line =
        new IntygCommon(
            correlationId,
            patientId,
            LocalDate.now(),
            IntygType.LISJP,
            line1.getEnhet().getId(),
            line1.getVardenhet().getId(),
            vg.getId(),
            line1.getKon(),
            event,
            line1.getDiagnoskod(),
            true,
            line1.getLakareId().getId());
