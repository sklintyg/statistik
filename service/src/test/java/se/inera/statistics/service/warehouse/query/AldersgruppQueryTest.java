...
public class AldersgruppQueryTest {
  ...
  public void one() {
    ...
    Map<Ranges.Range, Counter<Ranges.Range>> count =
        AldersgruppQuery.count(sjukfall, AldersgruppQuery.RANGES);
  ...
  public void useMax() {
    ...
    Mockito.when(widelineLoader.getAilesForVgs(any()))
        .thenReturn(Collections.singletonList(new Aisle(VARDGIVARE, facts)));
    Collection<Sjukfall> sjukfall = calculateSjukfallsHelper(warehouse.get(VARDGIVARE));
    List<OverviewChartRowExtended> count =
        AldersgruppQuery.getOverviewAldersgrupper(sjukfall, sjukfall);
