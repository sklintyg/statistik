...
public class NationellOverviewDataTest {
  ...
  @Mock private CountyPopulationManager countyPopulationManager;

  @InjectMocks private NationellOverviewData nationellOverviewData;
  ...
  public void getOverviewUnknownDxGroupIncludedInCurrentButNotInPreviousShouldBeHandledCorrectly() {
    ...
    final CountyPopulation countyPopulation =
        new CountyPopulation(Collections.emptyMap(), LocalDate.now());
    when(countyPopulationManager.getCountyPopulation(nullable(Range.class)))
        .thenReturn(countyPopulation);
    ...
    data.setOverviewDiagnosgrupperResult(
        new DiagnosgruppResponse(null, icdTyps, Arrays.asList(row1, row2)));
  ...
  public void shouldCalculateSickLeaveDifferenceInPercent() {
    final var expectedSickleaveDifference =
        12; // 8 / thousand -> 9 / thousand == 12.5 percent ~ 12 percent
  ...
  private SimpleKonResponse getOverviewLanResult(
      int female, int male, String county, String rowName) {
  ...
  private void setupCountyPopulation(int female, int male, String county) {
    ...
    final CountyPopulation countyPopulation =
        new CountyPopulation(countyPopulationMap, LocalDate.now());
    when(countyPopulationManager.getCountyPopulation(nullable(Range.class)))
        .thenReturn(countyPopulation);
  }
