...
public class RepresentativeIntygIT {
  ...
  @Autowired private QueueHelper queueHelper;
  ...
  public void deliver_document_from_in_queue_to_statistics_repository() throws IOException {
    ...
    int i = 0;
    for (TestIntyg intyg :
        getIntygWithHelsjukAndSingelmanadAndSingelDiagnos(
            getPerson(PERSON_K1950), getPerson(PERSON_K1960), getPerson(PERSON_M1979))) {
    ...
    Map<String, TestData> result =
        queueHelper.printAndGetPersistedData(
            getVardenhet(ENVE), getVardenhet(TVAVE), new Range(getStart(0), getStop(3)));
  ...
  @Test
  public void deliver_document_from_in_queue_to_statistics_repository_with_usecase_data()
      throws IOException {
    ...
    LOG.info("===========INPUT==========");
    List<String> persons =
        Arrays.asList(getPerson(PERSON_K1950), getPerson(PERSON_K1960), getPerson(PERSON_M1979));
    List<String> diagnosKods =
        Arrays.asList(G01, L99, A00, "F99", "R00", "Z00", "M00", "P00", "G02");
    List<LocalDate> start =
        Arrays.asList(
            LocalDate.parse("2012-02-05"),
            LocalDate.parse("2012-03-01"),
            LocalDate.parse("2012-02-05"),
            LocalDate.parse("2012-03-01"),
            LocalDate.parse("2013-02-05"),
    ...
    List<LocalDate> stop =
        Arrays.asList(
            ...
            LocalDate.parse("2012-02-07"),
            LocalDate.parse("2013-01-29"),
            LocalDate.parse("2013-02-08"),
            LocalDate.parse("2013-01-30"),
            LocalDate.parse("2013-02-09"),
            LocalDate.parse("2013-01-31"),
            LocalDate.parse("2013-02-10"));
    List<HsaIdVardgivare> vardgivares =
        Arrays.asList(
            new HsaIdVardgivare("EnVG"),
            new HsaIdVardgivare("TvaVG"),
            new HsaIdVardgivare("EnVG"),
            new HsaIdVardgivare("TvaVG"),
            new HsaIdVardgivare("EnVG"),
            new HsaIdVardgivare("TvaVG"),
    ...
    List<HsaIdEnhet> vardenhet =
        Arrays.asList(
            ...
            getVardenhet(TVAVE),
            getVardenhet(ENVE));
    List<TestIntyg> intygs =
        getIntygWithHelsjukAndSingelmanadAndDiagnosList(
            persons, diagnosKods, start, stop, vardgivares, vardenhet);
    int i = 1000;
  ...
  private List<TestIntyg> getIntygWithHelsjukAndSingelmanadAndSingelDiagnos(
      String... personNummers) {
  ...
  private List<TestIntyg> getIntygWithHelsjukAndSingelmanadAndDiagnosList(
      List<String> personNummers,
      List<String> diagnosKods,
      List<LocalDate> starts,
      List<LocalDate> stops,
      List<HsaIdVardgivare> vardgivares,
      List<HsaIdEnhet> vardenhets) {
    List<TestIntyg> testIntygs = new ArrayList<>();
    for (String person : personNummers) {
      for (int i = 0; i < diagnosKods.size(); i++) {
        ...
        testIntygs.add(
            new TestIntyg(person, getGrads(0), start, stop, vardenhet, vardgivare, diagnosKod));
      }
    }
    return testIntygs;
  }
  ...
  private LocalDate getStop(int i) {
    LocalDate[] dates = {
      LocalDate.parse("2012-01-22"),
      LocalDate.parse("2011-02-21"),
