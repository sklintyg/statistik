...
public class SimpleKonResponseTest {
  ...
  public void testCreateWithEmptyInput() {
    // Given
    final KonDataResponse diagnosgruppResponse =
        new KonDataResponse(
  ...
  public void testCreateGroupDataSummed() {
    ...
    final List<KonDataRow> rows =
        Arrays.asList(
            new KonDataRow("rowname1", Arrays.asList(new KonField(1, 2))),
            new KonDataRow("rowname2", Arrays.asList(new KonField(3, 4))));
    final KonDataResponse diagnosgruppResponse =
        new KonDataResponse(AvailableFilters.getForSjukfall(), groups, rows);
  ...
  public void testMerge() {
    ...
    responses.add(
        new SimpleKonResponse(
            AvailableFilters.getForSjukfall(), Arrays.asList(new SimpleKonDataRow("First", 1, 2))));
    responses.add(
        new SimpleKonResponse(
            AvailableFilters.getForSjukfall(),
            Arrays.asList(new SimpleKonDataRow("Second", 10, 20))));

    // When
    final SimpleKonResponse mergedResp =
        SimpleKonResponse.merge(responses, false, AvailableFilters.getForSjukfall());
  ...
  public void testMergeWithEqualGroupsWithMergeEqualRowsFlag() {
    ...
    // When
    final SimpleKonResponse mergedResp =
        SimpleKonResponse.merge(responses, true, AvailableFilters.getForSjukfall());
