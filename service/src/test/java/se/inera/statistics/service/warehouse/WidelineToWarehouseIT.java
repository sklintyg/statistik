...
public class WidelineToWarehouseIT {
  ...
  public void testPopulateWarehouse() throws Exception {
    ...
    Assert.assertEquals(
        Icd10.icd10ToInt("A00-B99", Icd10RangeType.KAPITEL), fact.getDiagnoskapitel());
    Assert.assertEquals(
        Icd10.icd10ToInt("A15-A19", Icd10RangeType.AVSNITT), fact.getDiagnosavsnitt());
    Assert.assertEquals(
        Icd10.icd10ToInt("A16", Icd10RangeType.KATEGORI), fact.getDiagnoskategori());
