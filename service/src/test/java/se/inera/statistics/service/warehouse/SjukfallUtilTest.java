...
public class SjukfallUtilTest {
  ...
  @InjectMocks private SjukfallUtil sjukfallUtil;
  ...
  public void twoCloseIntygForSamePersonIsOneSjukfall() throws Exception {
    ...
    final List<HsaIdLakare> lakare =
        sjukfall.getLakare().stream().map(lakare1 -> lakare1.getId()).collect(Collectors.toList());
  ...
  public void iterator() throws Exception {
    ...
    Iterator<SjukfallGroup> actives =
        sjukfallUtil
            .sjukfallGrupper(
                LocalDate.parse("2010-11-01"),
                3,
                1,
                aisle.createAisle(),
                createEnhetFilterFromInternalIntValues(2))
            .iterator();
  ...
  private Fact createFact(
      int enhet, int patient, int startdatum, int lakarintyg, int sjukskrivningslangd) {
  ...
  private Fact createFact(
    ...
    return aFact()
        ...
        .withSlutdatum(startdatum + sjukskrivningslangd - 1)
        .withLakarkon(FEMALE)
        .withLakaralder(32)
    ...
    Iterator<SjukfallGroup> actives =
        sjukfallUtil
            .sjukfallGrupper(
                ...
                1,
                aisle.createAisle(),
                createEnhetFilterFromInternalIntValues(ENHET1))
            .iterator();
  ...
  public void testSjukfallCacheWillOnlyReturnValuesForTheCorrectVg() throws Exception {
    ...
    // When
    final Iterable<SjukfallGroup> sjukfallGroups1 =
        sjukfallUtil.sjukfallGrupper(
            monthStart, 1, 1, aisle1.createAisle(), SjukfallUtil.ALL_ENHETER);
    final Iterable<SjukfallGroup> sjukfallGroups2 =
        sjukfallUtil.sjukfallGrupper(
            monthStart, 2, 1, aisle2.createAisle(), SjukfallUtil.ALL_ENHETER);
    final Iterable<SjukfallGroup> sjukfallGroups3 =
        sjukfallUtil.sjukfallGrupper(
            monthStart, 1, 1, aisle2.createAisle(), SjukfallUtil.ALL_ENHETER);
    final Iterable<SjukfallGroup> sjukfallGroups4 =
        sjukfallUtil.sjukfallGrupper(
            monthStart, 2, 1, aisle1.createAisle(), SjukfallUtil.ALL_ENHETER);
  ...
  public void testCalculateKonDataResponse() throws Exception {
    ...
    sjukfallGrupper.add(
        new SjukfallGroup(
            Range.createForLastMonthsExcludingCurrent(1, clock),
            Arrays.asList(
                createSjukfall(Kon.FEMALE), createSjukfall(Kon.MALE), createSjukfall(Kon.MALE))));
    final Aisle currentAisle = aisle.createAisle();
    Mockito.when(spy.sjukfallGrupper(start, periods, periodSize, currentAisle, filter))
        .thenReturn(sjukfallGrupper);

    // When
    final KonDataResponse response =
        spy.calculateKonDataResponse(
            currentAisle,
            filter,
            start,
            periods,
  ...
  public static FilterPredicates createEnhetFilterFromInternalIntValues(Integer... enhetIds) {
    final Set<HsaIdEnhet> availableEnhets =
        Arrays.stream(enhetIds)
            .map(integer -> new HsaIdEnhet(String.valueOf(integer)))
            .collect(Collectors.toSet());
    final String hashValue = FilterPredicates.getHashValueForEnhets(availableEnhets);
    return new FilterPredicates(
        fact -> availableEnhets.contains(fact.getVardenhet()), sjukfall -> true, hashValue, false);
  }

  private Collection<Sjukfall> calculateSjukfallsHelper() {
    return sjukfallUtil
        .sjukfallGrupper(
            LocalDate.of(2000, 1, 1), 1, 1000000, aisle.createAisle(), SjukfallUtil.ALL_ENHETER)
        .iterator()
        .next()
        .getSjukfall();
  }
