...
class JmsReceiverTest {
  ...
  @Mock private ProcessIntygsentLog processIntygsentLog;
  @Mock private ProcessMessageLog processMessageLog;
  @Mock private MdcHelper mdcHelper;
  @InjectMocks private JmsReceiver jmsReceiver;
  ...
  void testOnMessageIntygUnknowType() throws MessageNotWriteableException, IOException {
    ...
    Message rawMessage =
        getIntygMessage(intyg, correlationId, certificateType, JmsReceiver.CREATED);
  ...
  void testOnMessageRevokedIntygKnowType(String certificateType)
    ...
    Message rawMessage =
        getIntygMessage(intyg, correlationId, certificateType, JmsReceiver.REVOKED);

    jmsReceiver.onMessage(rawMessage);

    verify(receiver, times(1))
        .accept(
            Mockito.eq(EventType.REVOKED),
            Mockito.eq(intyg),
            Mockito.eq(correlationId),
            Mockito.anyLong());
  }
  ...
  void onMessageMessageSent() throws MessageNotWriteableException, IOException {
    ...
    verify(processMessageLog, times(1))
        .store(
            Mockito.eq(MessageEventType.SENT),
            Mockito.eq(messageText),
            Mockito.eq(messageId),
            Mockito.anyLong());
  }

  private Message getSentMessage(String messageText, String messageId)
      throws MessageNotWriteableException, IOException {
  ...
  private Message getSentIntygMessage(String correlationId, String recipient)
      throws MessageNotWriteableException, IOException {
  ...
  private Message getIntygMessage(
      String intyg, String correlationId, String certificateType, String action)
      throws MessageNotWriteableException, IOException {
