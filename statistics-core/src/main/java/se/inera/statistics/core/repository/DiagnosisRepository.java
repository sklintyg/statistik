...
public interface DiagnosisRepository extends JpaRepository<DiagnosisEntity, Long> {
	...
	@Query("select e.id from DiagnosisEntity as e where e.icd10Group = :icd10Group")
	List<Long> findIdsByIcd10group(@Param("icd10Group") final String icd10Group);
	
	//TODO: Look into better ordering. In current form, IX will come before V, VI, VII
	@Query("select e.icd10Group from DiagnosisEntity as e group by icd10Group order by icd10Group")
