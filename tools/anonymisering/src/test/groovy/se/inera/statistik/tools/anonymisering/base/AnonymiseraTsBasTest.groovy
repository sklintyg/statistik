...
class AnonymiseraTsBasTest {
    ...
    void testaAnonymiseringAvTsBas() {
        ...
        String expected = FileUtils.readFileToString(new ClassPathResource("/ts-bas_anonymized.xml").getFile(), "UTF-8")

        //When
        String document = FileUtils.readFileToString(new ClassPathResource("/ts-bas_not_anonymized.xml").getFile(), "UTF-8")
        String actual = anonymiseraTsBas.anonymisera(document)
