...
public class SjukfallPerLanConverterTest {
  ...
  public void convertCasesPerCountyDataTest() {
    ...
    SimpleKonResponse ageGroupsResponseNew =
        new SimpleKonResponse(AvailableFilters.getForSjukfall(), perCountyRows1);
    ...
    final CountyPopulation countyPopulation =
        new CountyPopulation(population, LocalDate.now(clock));

    CasesPerCountyConverter converter =
        new CasesPerCountyConverter(
            ageGroupsResponseNew, countyPopulation, new Range(fromNew, toNew));
    ...
    assertEquals(
        "[78, 40, 38, 3000, 600, 2400, 26,00, 66,67, 15,83]", rows.get(0).getData().toString());
    assertEquals(
        "[27, 13, 14, 1000, 200, 800, 27,00, 65,00, 17,50]", rows.get(1).getData().toString());
    assertEquals(
        "[39, 24, 15, 1000, 200, 800, 39,00, 120,00, 18,75]", rows.get(2).getData().toString());
    assertEquals(
        "[12, 3, 9, 1000, 200, 800, 12,00, 15,00, 11,25]", rows.get(3).getData().toString());
