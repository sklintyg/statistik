...
.factory('pdfFactory',
    ...
    function($window, $timeout, thousandseparatedFilter, $location, _, MAX_ROWS_TABLE_PDF, messageService, $rootScope,
      ...
      function _print($scope, charts) {
        ...
        if ($scope.useSpecialPrintTable) { //Used when table must be horizontally divided on separate pages
          ...
          $timeout(function() {
            ...
            _generate(headers, table, charts, $scope.activeEnhetsFilters, $scope.activeEnhetsFiltersAllSelected,
                $scope.activeDiagnosFilters, $scope.activeSjukskrivningslangdsFilters,
          ...
          _generate(headers, table, charts, $scope.activeEnhetsFilters, $scope.activeEnhetsFiltersAllSelected, $scope.activeDiagnosFilters,
              $scope.activeSjukskrivningslangdsFilters,
              $scope.activeAldersgruppFilters, $scope.activeIntygstypFilters, filename, pdfDoneCallback);
        }
      }
      ...
      function _generate(headers, table, charts, enhetsFilter, enhetsFilterAllSelected, diagnosFilter, sjukskrivningslangdFilter,
          aldersgruppFilter, intygstyperFilter, filename, pdfDoneCallback) {
        var content = [];
        ...
        _addAllFilters(content, enhetsFilterAllSelected, enhetsFilter, diagnosFilter, sjukskrivningslangdFilter, aldersgruppFilter,
            intygstyperFilter);
      ...
      function _addAllFilters(content, enhetsFilterAllSelected, enhetsFilter, diagnosFilter, sjukskrivningslangdFilter, aldersgruppFilter,
          intygstyperFilter) {
      ...
      function _updatePrintDataTable(headers, rows) {
        ...
        var firstColumnWidths = _.map(rows, function(row) {
          return calcWidth(row.name);
        });
