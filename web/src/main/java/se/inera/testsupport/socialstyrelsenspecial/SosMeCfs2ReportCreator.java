...
public class SosMeCfs2ReportCreator {
  ...
  public SosMeCfs2ReportCreator(
      Iterator<Aisle> aisles, SjukfallUtil sjukfallUtil, int cutoff, Icd10 icd10) {
    this.aisles = aisles;
  ...
  public List<SosMeCfs2Row> getSosReport() {
    ...
    for (Iterator<Aisle> it = aisles; it.hasNext(); ) {
      Aisle aisle = it.next();
      final Predicate<Sjukfall> sjukfallDxFilter =
          sjukfall -> g933 == sjukfall.getLastDx().getDiagnoskod();
      ...
      final Predicate<Sjukfall> sjukfallDateFilter =
          sjukfall -> sjukfall.getEnd() >= fromDay && sjukfall.getEnd() <= toDay;
      Predicate<Sjukfall> sjukfallFilter = sjukfallDxFilter.and(sjukfallDateFilter);
      final FilterPredicates filter =
          new FilterPredicates(f -> true, sjukfallFilter, "sosspecial3", true);
        ...
        if (sjukfalls.size() >= cutoff) { // Apply cutoff for second part of report
          for (Sjukfall sjukfall : sjukfalls) {
            ...
            final String ageGroup =
                AgeGroupSoc.getGroupForAge(age).orElse(AgeGroupSoc.GROUP1_0TO16).getGroupName();
            ...
            final List<Icd10RangeType> rangeTypes =
                Collections.singletonList(Icd10RangeType.KATEGORI);
      ...
      final Map<SosMeCfs2Tuple, List<SosMeCfs2ComputeRow>> vgRowsGroupedByGenderAndAgegroup =
          sosRowsOnVg.stream()
              .collect(Collectors.groupingBy(p -> new SosMeCfs2Tuple(p.getKon(), p.getAgeGroup())));
      for (List<SosMeCfs2ComputeRow> sosMeCfs2ComputeRows :
          vgRowsGroupedByGenderAndAgegroup.values()) {
        if (sosMeCfs2ComputeRows.size() >= cutoff) {
          sosRows.addAll(sosMeCfs2ComputeRows);
        }
      }
    }
    final Map<SosMeCfs2Tuple, List<SosMeCfs2ComputeRow>> groupedByGenderAndAgegroup =
        sosRows.stream()
            .collect(Collectors.groupingBy(p -> new SosMeCfs2Tuple(p.getKon(), p.getAgeGroup())));
    final List<SosMeCfs2Row> collect =
        groupedByGenderAndAgegroup.entrySet().stream()
            .map(
                sos3TupleListEntry -> {
                  final List<SosMeCfs2ComputeRow> rows = sos3TupleListEntry.getValue();
                  final List<Integer> lengths =
                      rows.stream()
                          .map(SosMeCfs2ComputeRow::getLength)
    ...
    collect.add(
        new SosMeCfs2Row(
            null, "Andel sjukfall med mer än 1 diagnos: " + shareWithSeveralDxs, 0, 0, 0, 0));
    final Map<Integer, Long> countedDxs =
        dxs.stream().collect(Collectors.groupingBy(t -> t, Collectors.counting()));
    final int maxNumberOfDxsToShow = 10;
    final List<Map.Entry<Integer, Long>> mostCombinedDxs =
        countedDxs.entrySet().stream()
            .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
            .filter(r -> r.getKey() != g93)
            .limit(maxNumberOfDxsToShow)
            .collect(Collectors.toList());
