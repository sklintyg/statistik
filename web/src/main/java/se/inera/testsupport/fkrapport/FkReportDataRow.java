...
/**
 * Placeholder for the given combination of diagnos, kon and lan. It will only accept FkFactRow data
 * that fits it's combination. The actual measurements / average, medians etc can be queried at any
 * time, but in reality it's only done when all facts have been distributed.
 */
public class FkReportDataRow {
  ...
  public void ingest(FkFactRow factRow) {
    ...
    if (matchesDiagnose(factRow.getDiagnos())
        && matchesKon(factRow.getKon())
        && matchesLan(factRow.getLanId())) {
