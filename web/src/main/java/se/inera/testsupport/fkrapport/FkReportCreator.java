...
public class FkReportCreator {
  ...
  public FkReportCreator(
      Iterator<Aisle> aisles, Icd10 icd10, List<String> diagnoseCategories, Clock clock) {
    this.aisles = aisles;
    this.icd10 = icd10;
  ...
  List<FkReportDataRow> createResultRowsForDiagnoses() {
    ...
    diagnoseEntries.forEach(
        diagnoseEntry ->
            lan.forEach(
                lansId -> {
                  rows.add(
                      new FkReportDataRow(
                          diagnoseEntry.getDiagnoseValue(),
                          diagnoseEntry.getDiagnoseMatcher(),
                          Kon.FEMALE,
                          lansId));
                  rows.add(
                      new FkReportDataRow(
  ...
  private List<DiagnoseEntry> buildDiagnoseEntries() {
    ...
    diagnoseCategories.forEach(
        diagnoseCategory -> {
          ...
          result.add(
              new DiagnoseEntry(diagnoseCategory + "+", diagnoseCategory + MATCH_ANYTHING_REGEXP));
  ...
  private List<DiagnoseEntry> getExactMatchesForSubdiagnoses(String diagnoseCategory) {
    ...
    if (kategori == null) {
      throw new IllegalArgumentException(
          "Diagnose category " + diagnoseCategory + " not found in ICD10 registry");
    }
    List<DiagnoseEntry> result = new ArrayList<>();
  ...
  private List<FkFactRow> getEffectiveFactRows() {
    ...
    final int toIntDay =
        WidelineConverter.toDay(getLastDateOfLastYear()); // dagnr för 2015-12-31 relativt
    // 2000-01-01
    ...
    return StreamSupport.stream(iterableAisles.spliterator(), true)
        .flatMap(
            aisle ->
                getFkFactRowsPerVg(range, fromIntDay, toIntDay, SjukfallUtil.ALL_ENHETER, aisle))
        .collect(Collectors.toList());
  }

  private Stream<FkFactRow> getFkFactRowsPerVg(
      Range range, int fromIntDay, int toIntDay, FilterPredicates sjukfallFilter, Aisle aisle) {
    LOG.info("About to get sjukfall for vg  " + aisle.getVardgivareId());
    // Hämta ut dom som hör till detta år
    final ArrayList<SjukfallGroup> sjukfallGroups =
        Lists.newArrayList(
    ...
    return sjukfallGroups.stream()
        ...
        .collect(Collectors.toMap(Sjukfall::getFirstIntygId, p -> p, (p, q) -> p))
        .values()
        ...
        .filter(
            sjukfall ->
                inPeriod(
                    getAllFactsInIntyg(sjukfall.getFirstIntygId(), aisle), fromIntDay, toIntDay))
  ...
  private boolean inPeriod(Stream<Fact> intygFacts, int from, int to) {
    final StartEnd intyg =
        intygFacts.reduce(
            new StartEnd(Integer.MAX_VALUE, Integer.MIN_VALUE),
            (point, fact) -> {
