...
public class TestIntygInjector {
  ...
  private static final List<HsaIdVardgivare> VG =
      Arrays.asList(
          new HsaIdVardgivare("vg1"),
          new HsaIdVardgivare("vg2"),
          new HsaIdVardgivare("vg3"),
          new HsaIdVardgivare("vg4"),
          new HsaIdVardgivare("vg5"));

  private static final List<HsaIdLakare> LAKARE = new ArrayList<>();

  private static final List<String> DIAGNOSER = new ArrayList<>();
  private static final Multimap<HsaIdVardgivare, HsaIdEnhet> VARDGIVARE =
  ...
  @Autowired private RestSupportService restSupportService;
  ...
  private void publishUtlatanden() {
    ...
    try {
      template =
          IOUtils.toString(
              getClass().getClassLoader().getResourceAsStream("testsupport/sendMessageToCare.xml"));
    } catch (IOException e) {
    ...
    LOG.info(
        "Inserting "
            + personNummers.size()
            + " certificates completed. Use -Dstatistics.test.max.intyg=<x> to limit inserts.");
  }

  private String createAndInsertIntyg(UtlatandeBuilder builder, String patientId) {
    // CHECKSTYLE:OFF MagicNumber
    LocalDate start =
        base.plusMonths(random.nextInt(MONTHS)).plusDays(random.nextInt(SHORT_PERIOD_DAYS));
    LocalDate end =
    ...
    final JsonNode data =
        builder.build(patientId, start, end, lakare, vardenhet, vardgivare, diagnos, arbetsformaga);
    ...
    final String huvudenhetId =
        start.getDayOfYear() % 2 > 0
            ? random(Lists.newArrayList(VARDGIVARE.get(vardgivare))).getId()
            : null;
  ...
  private List<String> readList(final String path, final int maxIntyg) {
    try (BufferedReader in =
        new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(path), "utf8"))) {
      List<String> list = new ArrayList<>();
      int count = 0;
      for (String line = in.readLine();
          line != null && (maxIntyg == 0 || count++ < maxIntyg);
          line = in.readLine()) {
        list.add(line);
      }
      return list;
