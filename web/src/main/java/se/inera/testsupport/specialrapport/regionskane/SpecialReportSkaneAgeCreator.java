...
public class SpecialReportSkaneAgeCreator {
  ...
  public List<SpecialSkaneRow> getReport(boolean includeOngoingSjukfall, List<String> enhetsIn) {
    ...
    for (Iterator<Aisle> it = aisles; it.hasNext(); ) {
      ...
      for (Range period : periods) {
        ...
        final Predicate<Fact> intygFilter =
            in -> enhetStringIds.contains(in.getVardenhet().getId());
        ...
        final Iterable<SjukfallGroup> sjukfallGroups =
            sjukfallUtil.sjukfallGrupper(
                period.getFrom(), 1, period.getNumberOfMonths(), aisle, filter);
        ...
        for (SjukfallGroup sjukfallGroup : sjukfallGroups) {
          ...
          for (Sjukfall sjukfall : sjukfalls) {
            if (includeOngoingSjukfall || sjukfall.getStart() >= startDayOfRange) {
              ...
              final int ageDay =
                  includeOngoingSjukfall
                      ? Math.min(sjukfall.getEnd(), WidelineConverter.toDay(currentRange.getTo()))
                      : sjukfall.getStart();
              ...
              final Optional<HsaIdEnhet> enhet =
                  enhetsInSjukfall.stream().filter(s -> enhets.contains(s)).findAny();
              if (enhet.isPresent()) {
                final Kon kon = sjukfall.getKon();
                final SpecialSkaneAgeComputeRow sosRow =
                    new SpecialSkaneAgeComputeRow(age, enhet.get(), period, kon);
                rows.add(sosRow);
              }
            }
          }
    ...
    results.addAll(
        resultadder(
            ...
            r ->
                r.getEnhet()
                    + r.getRange().getFrom().format(DateTimeFormatter.ISO_DATE)
                    + r.getKon().name(),
            o ->
                "Genomsnittlig ålder för "
                    + (includeOngoingSjukfall ? "pågående" : "nya")
                    + " sjukfall "
                    + SpecialReportSkaneCreator.getGenderName(o.getKon())
                    + " "
                    + o.getFormattedRange()));
  ...
  private List<SpecialSkaneRow> resultadder(
      List<SpecialSkaneAgeComputeRow> rows,
      Function<SpecialSkaneAgeComputeRow, String> groupByFunction,
      Function<SpecialSkaneAgeComputeRow, String> rowName) {
    final List<SpecialSkaneRow> results = new ArrayList<>();
    final Map<String, List<SpecialSkaneAgeComputeRow>> rowsGroupedByFunction =
        rows.stream().collect(Collectors.groupingBy(groupByFunction));
    for (Map.Entry<String, List<SpecialSkaneAgeComputeRow>> entry :
        rowsGroupedByFunction.entrySet()) {
      final SpecialSkaneAgeComputeRow firstRow = entry.getValue().get(0);
      final int missingAge = -2;
      results.add(
          new SpecialSkaneRow(
              rowName.apply(firstRow),
              firstRow.getEnhet().getId(),
              entry.getValue().stream()
                  .mapToInt(SpecialSkaneAgeComputeRow::getAge)
                  .average()
                  .orElse(missingAge)));
    }
    return results;
