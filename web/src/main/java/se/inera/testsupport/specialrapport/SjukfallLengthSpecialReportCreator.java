...
public class SjukfallLengthSpecialReportCreator {
  ...
  public SjukfallLengthSpecialReportCreator(
      Iterator<Aisle> aisles, SjukfallUtil sjukfallUtil, Icd10 icd10) {
    this.aisles = aisles;
  ...
  public List<SjukfallLengthSpecialRow> getReport(int year) {
    ...
    for (Iterator<Aisle> it = aisles; it.hasNext(); ) {
      ...
      final FilterPredicates filter =
          new FilterPredicates(f -> true, s -> true, "SpecialLength", true);
      ...
      final Iterable<SjukfallGroup> sjukfallGroups =
          sjukfallUtil.sjukfallGrupper(
              range.getFrom(), 1, range.getNumberOfMonths() + 1, aisle, filter);
      ...
      for (SjukfallGroup sjukfallGroup : sjukfallGroups) {
        ...
        for (Sjukfall sjukfall : sjukfalls) {
          if (sjukfall.getEnd() <= endDay) {
            ...
            final SjukfallLengthSpecialComputeRow sosRow =
                new SjukfallLengthSpecialComputeRow(kat, lengthGroup);
            rows.add(sosRow);
          }
        }
      }
    ...
    final Map<SjukfallsLangdGroupSpecial, List<SjukfallLengthSpecialComputeRow>>
        rowsGroupedByLength = getrowsGroupedByLength(rows);
    final List<SjukfallLengthSpecialRow> results =
        rowsGroupedByLength.entrySet().stream()
            .map(p -> new SjukfallLengthSpecialRow(p.getKey().getGroupName(), p.getValue().size()))
            .collect(Collectors.toList());

    final List<SjukfallsLangdGroupSpecial> groups91To180 =
        Collections.singletonList(SjukfallsLangdGroupSpecial.GROUP6_91TO180);
    results.addAll(
        getSjukfallLengthSpecialRows(groups91To180, "91-180 dagar", rowsGroupedByLength));
    final List<SjukfallsLangdGroupSpecial> groupsMoreThan180 =
        Arrays.asList(
            SjukfallsLangdGroupSpecial.GROUP7_181TO365, SjukfallsLangdGroupSpecial.GROUP8_366PLUS);
    results.addAll(
        getSjukfallLengthSpecialRows(groupsMoreThan180, "över 180 dagar", rowsGroupedByLength));
    return results;
  }

  private Map<SjukfallsLangdGroupSpecial, List<SjukfallLengthSpecialComputeRow>>
      getrowsGroupedByLength(List<SjukfallLengthSpecialComputeRow> rows) {
    final Map<SjukfallsLangdGroupSpecial, List<SjukfallLengthSpecialComputeRow>>
        rowsGroupedByLength =
            rows.stream()
  ...
  private List<SjukfallLengthSpecialRow> getSjukfallLengthSpecialRows(
      Collection<SjukfallsLangdGroupSpecial> groups,
      String groupName,
      Map<SjukfallsLangdGroupSpecial, List<SjukfallLengthSpecialComputeRow>> rowsGroupedByLength) {
    long totalNumberOfSjukfallInGroups =
        rowsGroupedByLength.entrySet().stream()
            .filter(
                sjukfallsLangdGroupSpecialListEntry ->
                    groups.contains(sjukfallsLangdGroupSpecialListEntry.getKey()))
            .mapToLong(
                sjukfallsLangdGroupSpecialListEntry ->
                    sjukfallsLangdGroupSpecialListEntry.getValue().size())
            .sum();
    ...
    final List<String> topList =
        rowsGroupedByLength.entrySet().stream()
            ...
            .flatMap(
                (Function<List<SjukfallLengthSpecialComputeRow>, Stream<Integer>>)
                    calcRows -> calcRows.stream().map(SjukfallLengthSpecialComputeRow::getKat))
            ...
            .map(
                amount ->
                    icd10.findIcd10FromNumericId(amount.getKey()).getVisibleId()
                        + " med "
                        + amount.getValue()
                        + " förekomster"
                        + " ("
                        ...
                        + "% av "
                        + totalNumberOfSjukfallInGroups
                        + ")")
            .collect(Collectors.toList());
    for (int i = 0; i < topList.size(); i++) {
      final String s = topList.get(i);
      rows.add(
          new SjukfallLengthSpecialRow(
              "Vanligaste diagnos i sjukfall " + groupName + " #" + (i + 1) + " är " + s, 0));
    }
    return rows;
  }
