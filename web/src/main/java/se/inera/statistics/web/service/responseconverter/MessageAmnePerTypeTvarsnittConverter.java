...
public class MessageAmnePerTypeTvarsnittConverter extends MultiDualSexConverter {
  ...
  public SimpleDetailsData convert(KonDataResponse data, FilterSettings filterSettings) {
    final List<MsgAmne> groups =
        data.getGroups().stream().map(MsgAmne::parse).collect(Collectors.toList());
    ...
    final KonDataResponse konDataResponse =
        new KonDataResponse(data.getAvailableFilters(), convertGroupNamesToText(groups), rows);
    final DualSexStatisticsData dssd =
        super.convert(konDataResponse, filterSettings, null, "%1$s", COLORS);
    final ChartData chartData =
        MessageAmnePerTypeConverter.merge(dssd.getFemaleChart(), dssd.getMaleChart());
