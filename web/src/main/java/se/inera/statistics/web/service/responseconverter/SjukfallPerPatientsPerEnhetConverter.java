...
public class SjukfallPerPatientsPerEnhetConverter {
  ...
  public SjukfallPerPatientsPerEnhetConverter(
      List<RegionEnhet> regionEnhets, List<HsaIdEnhet> connectedEnhetIds) {
  ...
  public SimpleDetailsData convert(
    ...
    final List<Message> combinedMessage =
        Converters.combineMessages(filterSettings.getMessage(), message);
  ...
  private TableData convertToTableData(List<SimpleKonDataRow> list) {
    Collections.sort(
        list,
        new Comparator<SimpleKonDataRow>() {
          @Override
    ...
    for (SimpleKonDataRow row : list) {
      ...
      if (listadePatienter != null) {
        ...
        final float sjukfallPerThousandListadePatienter =
            antalSjukfall / (listadePatienter / thousand);
        final String sjukfallPerThousandListadePatienterRounded =
            roundToTwoDecimalsAndFormatToString(sjukfallPerThousandListadePatienter);
        data.add(
            new NamedData(
                row.getName(),
                Arrays.asList(
                    antalSjukfall, listadePatienter, sjukfallPerThousandListadePatienterRounded),
                isMarked(row)));
      }
    }
    return TableData.createWithSingleHeadersRow(
        data,
        Arrays.asList(
            MessagesText.REPORT_VARDENHET,
            MessagesText.REPORT_ANTAL_SJUKFALL,
            MessagesText.REPORT_ANTAL_LISTNINGAR,
            MessagesText.REPORT_ANTAL_SJUKFALL_PER_1000_LISTNINGAR));
  }
  ...
  private ChartData convertToChartData(SimpleKonResponse casesPerMonth) {
    ...
    final ArrayList<EnhetWithPatients> enhetsWithPatients =
        toSortedEnhetsWithPatients(casesPerMonth);
    ...
    for (EnhetWithPatients enhet : enhetsWithPatients) {
      ...
      final float sjukfallPerThousandListadePatienter =
          enhet.getSjukfallPerThousandListadePatienter();
  ...
  private ArrayList<EnhetWithPatients> toSortedEnhetsWithPatients(SimpleKonResponse casesPerMonth) {
    ...
    Collections.sort(
        enhetsWithPatients,
        new Comparator<EnhetWithPatients>() {
          @Override
          public int compare(EnhetWithPatients o1, EnhetWithPatients o2) {
            return Float.compare(
                o2.getSjukfallPerThousandListadePatienter(),
                o1.getSjukfallPerThousandListadePatienter());
          }
        });
