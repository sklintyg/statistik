...
public class DiagnosisSubGroupsConverter {
  ...
  public DualSexStatisticsData convert(
      DiagnosgruppResponse diagnosisGroups, FilterSettings filterSettings, Message message) {
  ...
  private ChartData extractChartData(DiagnosgruppResponse data, List<Integer> topIndexes, Kon sex) {
    ...
    final List<ChartCategory> categories =
        data.getPeriods().stream().map(ChartCategory::new).collect(Collectors.toList());
  ...
  private List<ChartSeries> getTopColumns(KonDataResponse data, List<Integer> topIndexes, Kon sex) {
    ...
    if (topIndexes.isEmpty()) {
      topColumns.add(
          new ChartSeries(MessagesText.REPORT_GROUP_TOTALT, createList(data.getRows().size(), 0)));
      return topColumns;
    }
  ...
  private static List<Integer> sumRemaining(
      List<Integer> topIndexes, KonDataResponse data, Kon sex) {
  ...
  static List<Integer> getTopColumnIndexes(SimpleKonResponse simpleKonDataRowSimpleKonResponse) {
    ...
    final List<Integer> sortedIndexes =
        getColumnIndexesSortedBySum(simpleKonDataRowSimpleKonResponse);
    ...
    if (sortedIndexes.size() == MAX_NUMBER_OF_CHART_SERIES) {
      ...
      final ArrayList<Integer> sortedIndexesSubList =
          new ArrayList<>(sortedIndexes.subList(0, MAX_NUMBER_OF_CHART_SERIES - 1));
  ...
  private static List<Integer> getColumnIndexesSortedBySum(
      SimpleKonResponse simpleKonDataRowSimpleKonResponse) {
  ...
  private static List<Pair<Integer, Integer>> getIndexedSums(
      SimpleKonResponse simpleKonDataRowSimpleKonResponse) {
