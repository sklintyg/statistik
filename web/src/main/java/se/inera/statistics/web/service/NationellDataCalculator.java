...
/**
 * Invokes calculation of national statistics and converts the results into the correct format for
 * the reports.
 */
@Component
public class NationellDataCalculator {
  ...
  @Autowired private NationellOverviewData overviewData;

  @Autowired private CountyPopulationManager countyPopulationManager;
  ...
  private SimpleDetailsData buildNumberOfCasesPerMonth(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getAntalIntygRange());
  ...
  private DualSexStatisticsData buildDiagnosgrupper(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getDiagnosgrupperRange());
  ...
  private Map<String, DiagnosisSubGroupStatisticsData> buildDiagnoskapitel(NationellDataInfo data) {
    ...
    final Map<Icd10.Kapitel, DiagnosgruppResponse> diagnosavsnittResult =
        data.getDiagnosavsnittResult();
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getDiagnosavsnittRange());
    for (Map.Entry<Icd10.Kapitel, DiagnosgruppResponse> entry : diagnosavsnittResult.entrySet()) {
      ...
      final DualSexStatisticsData dssd =
          new DiagnosisSubGroupsConverter().convert(diagnosisGroup, filterSettings);
  ...
  private SimpleDetailsData buildAldersgrupper(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getAldersgrupperRange());
  ...
  private DualSexStatisticsData buildSjukskrivningsgrad(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getSjukskrivningsgradRange());
  ...
  private SimpleDetailsData buildSjukfallslangd(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getSjukfallslangdRange());
  ...
  private TableDataReport buildIntygPerTyp(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings = new FilterSettings(Filter.empty(), range);
    return new SimpleMultiDualSexConverter("Antal intyg totalt")
        .convert(intygPerTyp, filterSettings);
  }
  ...
  private SimpleDetailsData buildIntygPerSjukfall(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings =
        new FilterSettings(Filter.empty(), data.getIntygPerSjukfallRange());
  ...
  private TableDataReport buildKompletteringarPerFraga(NationellDataInfo data) {
    ...
    final FilterSettings filterSettings = new FilterSettings(Filter.empty(), range);
    return SimpleDualSexConverter.newGenericKompletteringarTvarsnitt()
        .convert(kompletteringar, filterSettings);
  }
}
