...
abstract class MultiDualSexConverter {
  ...
  DualSexStatisticsData convert(
      KonDataResponse dataIn,
  ...
  DualSexStatisticsData convert(
    ...
    KonDataResponse data =
        dataIn.getGroups().isEmpty() ? createEmptyResponse(dataIn.getAvailableFilters()) : dataIn;
  ...
  private KonDataResponse createEmptyResponse(AvailableFilters availableFilters) {
    ...
    final List<KonDataRow> rows =
        Collections.singletonList(new KonDataRow(MessagesText.REPORT_GROUP_TOTALT, data));
    return new KonDataResponse(availableFilters, groups, rows);
  }

  private ChartData extractChartData(
      KonDataResponse data, Kon sex, String seriesNameTemplate, Map<String, String> colors) {
  ...
  private List<ChartSeries> getChartSeries(
    ...
    for (int i = 0; i < data.getGroups().size(); i++) {
      ...
      final String seriesName =
          "%1$s".equals(seriesNameTemplate)
              ? groupName
              : String.format(seriesNameTemplate, groupName);
