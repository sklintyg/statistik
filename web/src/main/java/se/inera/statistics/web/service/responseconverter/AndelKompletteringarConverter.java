...
public class AndelKompletteringarConverter extends MultiDualSexConverter {
  ...
  public DualSexStatisticsData convert(KonDataResponse data, FilterSettings filterSettings) {
    final List<IntygType> groups =
        data.getGroups().stream().map(IntygType::parseString).collect(Collectors.toList());
    ...
    final KonDataResponse konDataResponse =
        new KonDataResponse(
            data.getAvailableFilters(), convertGroupNamesToText(groups), updateGenderFields(rows));
                          ...
                          konField -> {
                            ...
                            if (!(extrasObj instanceof AndelExtras)) {
                              throw new RuntimeException(
                                  "Wrong extras type, expected AndelExtras but got "
                                      + extrasObj.toString());
                            }
                            final AndelExtras extras = (AndelExtras) extrasObj;
                            final int f =
                                extras.getFemaleIntyg() == 0
                                    ? 0
                                    : percentageConvertion
                                        * extras.getFemaleKompl()
                                        / extras.getFemaleIntyg();
                            final int m =
                                extras.getMaleIntyg() == 0
                                    ? 0
                                    : percentageConvertion
                                        * extras.getMaleKompl()
                                        / extras.getMaleIntyg();
  ...
  protected Object getRowSum(KonDataRow row) {
    ...
    final BiFunction<AndelExtras, KonField, AndelExtras> andelExtraExtractor =
        (andelExtras, konField) -> {
          if (konField.getExtras() instanceof AndelExtras) {
            return AndelExtras.combined(andelExtras, (AndelExtras) konField.getExtras());
          }
          return andelExtras;
        };
    final AndelExtras total =
        row.getData().stream().reduce(emptyAndelExtras, andelExtraExtractor, AndelExtras::combined);
    final int tot =
        total.getWhole() == 0 ? 0 : PERCENTAGE_CONVERT * total.getPart() / total.getWhole();
    return tot + "%";
  }
  ...
  protected List<Object> getMergedSexData(KonDataRow row) {
    ...
    for (KonField konField : row.getData()) {
      final AndelExtras tot =
          konField.getExtras() instanceof AndelExtras
              ? (AndelExtras) konField.getExtras()
              : new AndelExtras(0, 0, 0, 0);
      final int total =
          tot.getWhole() == 0 ? 0 : PERCENTAGE_CONVERT * tot.getPart() / tot.getWhole();
      data.add(total + "%");
  ...
  TableData convertTable(KonDataResponse resp, String seriesNameTemplate) {
    ...
    final List<List<TableHeader>> newHeaders =
        tableData.getHeaders().stream()
            .map(
                row -> row.stream().map(this::getTableHeaderWithTitle).collect(Collectors.toList()))
            .collect(Collectors.toList());
  ...
  private TableHeader getTableHeaderWithTitle(TableHeader tableHeader) {
    return new TableHeader(
        tableHeader.getText(),
        tableHeader.getColspan(),
        getTitle(tableHeader),
        tableHeader.getMeta());
  }
