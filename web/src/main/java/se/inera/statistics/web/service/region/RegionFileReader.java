...
public class RegionFileReader {
  ...
  public List<RegionEnhetFileDataRow> readExcelData(DataSource dataSource)
      throws RegionEnhetFileParseException {
        ...
        if (!id.getId().isEmpty()) {
          if (!addedEnhetIds.add(id)) {
            throw new RegionEnhetFileParseException(
                messagePrefix + "Vårdenheten förekommer mer än en gång");
          }
          if (!id.getId().matches("^[a-zA-Z0-9-]+$")) {
            throw new RegionEnhetFileParseException(
                messagePrefix + "Kolumn “HSA-id” innehåller otillåtna tecken");
          }
  ...
  private Integer getListedPatients(String messagePrefix, Row row)
      throws RegionEnhetFileParseException {
      ...
      switch (cellPatients.getCellType()) {
        case NUMERIC:
          ...
          if (DoubleMath.isMathematicalInteger(patientsDouble)) {
            ...
            throw new RegionEnhetFileParseException(
                messagePrefix
                    + "Kolumn “Antal listade patienter” innehåller inte ett heltal: "
                    + patientsDouble);
          }
          break;
        case STRING:
