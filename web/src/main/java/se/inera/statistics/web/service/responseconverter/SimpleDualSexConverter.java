...
public class SimpleDualSexConverter {
  ...
  public SimpleDetailsData convert(
      SimpleKonResponse casesPerMonthIn, FilterSettings filterSettings, Message message) {
    TableData tableData = convertToTableData(casesPerMonthIn.getRows());
    SimpleKonResponse casesPerMonth =
        casesPerMonthIn.getRows().isEmpty()
            ? new SimpleKonResponse(
                casesPerMonthIn.getAvailableFilters(),
                Arrays.asList(new SimpleKonDataRow(MessagesText.REPORT_GROUP_TOTALT, 0, 0)))
            : casesPerMonthIn;
    ...
    return new SimpleDetailsData(
        tableData, chartData, range.toString(), availableFilters, filterResponse, combinedMessage);
  }
  ...
  protected TableData convertToTableData(List<SimpleKonDataRow> list) {
    ...
    return TableData.createWithSingleHeadersRow(
        data, Arrays.asList(tableGroupTitle, totalColumnName, femaleColumnName, maleColumnName));
  }
  ...
  protected ChartData convertToChartData(SimpleKonResponse casesPerMonth) {
    ...
    series.add(
        new ChartSeries(
            MessagesText.REPORT_GROUP_FEMALE, casesPerMonth.getDataForSex(Kon.FEMALE), Kon.FEMALE));
    series.add(
        new ChartSeries(
            MessagesText.REPORT_GROUP_MALE, casesPerMonth.getDataForSex(Kon.MALE), Kon.MALE));
