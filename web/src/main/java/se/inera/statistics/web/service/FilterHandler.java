...
public class FilterHandler {
  ...
  public List<HsaIdEnhet> getEnhetsFilterIds(
      Collection<HsaIdEnhet> filteredEnhets, HttpServletRequest request) {
    if (filteredEnhets == null) {
      final LoginInfo info = loginServiceUtil.getLoginInfo();
      final List<Verksamhet> businesses =
          info.getBusinessesForVg(getSelectedVgIdForLoggedInUser(request));
      return businesses.stream().map(Verksamhet::getId).collect(Collectors.toList());
    }
    return new ArrayList<>(filteredEnhets);
  }

  public FilterSettings getFilterForRegion(
      HttpServletRequest request, String filterHash, int defaultRangeValue) {
    if (filterHash == null || filterHash.isEmpty()) {
      return new FilterSettings(
          getFilterForAllAvailableEnhetsRegion(request),
    ...
    try {
      ...
      LOG.warn(
          "Could not use selected region filter. Falling back to default filter. Msg: "
              + e.getMessage());
      ...
      return new FilterSettings(
          getFilterForAllAvailableEnhets(request, true),
          Range.createForLastMonthsIncludingCurrent(defaultRangeValue, clock),
  ...
  public List<Verksamhet> getAllVerksamhetsForLoggedInRegionsUser(HttpServletRequest request) {
    ...
    return enhets.stream()
        .map(enhet -> loginServiceUtil.enhetToVerksamhet(enhet))
        .collect(Collectors.toList());
  }

  public FilterSettings getFilter(
      HttpServletRequest request, String filterHash, int defaultNumberOfMonthsInRange) {
  ...
  public FilterSettings getFilter(
      ...
      String filterHash,
      int defaultNumberOfMonthsInRange,
      boolean vardenhetsdepth) {
    try {
      if (filterHash == null || filterHash.isEmpty()) {
        return new FilterSettings(
            getFilterForAllAvailableEnhets(request, vardenhetsdepth),
      ...
      return getFilterSettings(
          request, filterHash, defaultNumberOfMonthsInRange, inFilter, enhetsIDs, vardenhetsdepth);
    } catch (FilterException | FilterHashException e) {
      LOG.warn(
          "Could not use selected filter. Falling back to default filter. Msg: " + e.getMessage());
  ...
  private FilterSettings getFilterSettings(
      HttpServletRequest request,
      String filterHash,
      int defaultRangeValue,
      FilterData inFilter,
      List<HsaIdEnhet> enhetsInFilter,
      boolean veDepth)
    ...
    final Predicate<Fact> enhetFilter =
        sjukfallUtil.createEnhetFilter(enheter.toArray(new HsaIdEnhet[0])).getIntygFilter();
    return getFilterSettings(
        filterHash, defaultRangeValue, inFilter, enheter, enhetFilter, veDepth);
  }

  private FilterSettings getFilterSettingsRegion(
      HttpServletRequest request,
      String filterHash,
      int defaultRangeValue,
      FilterData inFilter,
      ArrayList<HsaIdEnhet> enhetsInFilter)
      throws FilterException, TooEarlyEndDateException, TooLateStartDateException {
  ...
  private FilterSettings getFilterSettings(
      ...
      int defaultRangeValue,
      FilterData inFilter,
      Collection<HsaIdEnhet> enhetsIDs,
      Predicate<Fact> enhetFilter,
      boolean veDepth)
      throws FilterException, TooEarlyEndDateException, TooLateStartDateException {
    ...
    final FilterPredicates sjukfallFilter =
        new FilterPredicates(predicate, sjukfallLengthFilter, hash, sjukfallangdfilterActive);
    final Filter filter =
        new Filter(
            sjukfallFilter,
            enhetsIDs,
            diagnoser,
            filterDataToReadableSjukskrivningslangdName(inFilter),
            toReadableAgeGroupNames(aldersgrupp),
            filterHash,
            filterDataToReadableIntygTypeName(inFilter),
  ...
  private List<String> toReadableIntygTypeName(List<String> intygstyper) {
    if (intygstyper == null) {
      return IntygType.getInIntygtypFilter().stream()
          .map(IntygType::getText)
          .collect(Collectors.toList());
    }
    return intygstyper.stream()
        .map(
            intygstyp ->
                IntygType.parseStringOptional(intygstyp)
  ...
  private List<String> toReadableSjukskrivningslangdName(List<String> sjukskrivningslangds) {
    if (sjukskrivningslangds == null) {
      return Arrays.stream(SjukfallsLangdGroup.values())
          .map(SjukfallsLangdGroup::getGroupName)
          .collect(Collectors.toList());
    }
    return sjukskrivningslangds.stream()
        .map(
            sjukskrivningslangd ->
                SjukfallsLangdGroup.parse(sjukskrivningslangd)
  ...
  private Predicate<Sjukfall> getSjukfallLengthFilter(List<String> filterLangds) {
    ...
    final List<SjukfallsLangdGroup> filteredLangdGroups =
        filterLangds.stream()
    ...
    return sjukfall -> {
      ...
      return filteredLangdGroups.stream()
          .anyMatch(group -> group.getFrom() <= realDays && group.getTo() >= realDays);
    };
  }
  ...
  private RangeMessageDTO getRange(FilterData inFilter, int defaultRangeValue)
    ...
    if (inFilter.isUseDefaultPeriod()) {
      return new RangeMessageDTO(
          Range.createForLastMonthsIncludingCurrent(defaultRangeValue, clock), null);
    }
    ...
    if (fromDate == null || toDate == null) {
      throw new FilterException(
          "Can not parse null range dates. From: " + fromDate + ", To: " + toDate);
    }
    try {
      ...
      throw new FilterException(
          "Could not parse range dates. From: " + fromDate + ", To: " + toDate, e);
    }
  }

  private RangeMessageDTO validateAndGetFilterRangeMessage(
      LocalDate originalFrom, LocalDate originalTo)
      throws FilterException, TooEarlyEndDateException, TooLateStartDateException {
    String message = null;
    LocalDate from = originalFrom;
    LocalDate to = originalTo;

    LocalDate highestAcceptedEndDate =
        LocalDate.now(clock).plusMonths(1).withDayOfMonth(1).minusDays(1);
    ...
    if (isBefore && isAfter) {
      ...
      message =
          String.format(
              MessagesText.FILTER_WRONG_FROM_AND_END_DATE,
              formattedLowestStartDate,
              formattedHighestEndDate,
              formattedLowestStartDate,
              formattedHighestEndDate);
    } else if (isBefore) {
      if (to.isBefore(LOWEST_ACCEPTED_START_DATE)) {
        ...
        message =
            String.format(
                MessagesText.FILTER_WRONG_FROM_DATE, LOWEST_ACCEPTED_START_DATE.format(formatter));
      }
      ...
      message =
          String.format(
              MessagesText.FILTER_WRONG_END_DATE, highestAcceptedEndDate.format(formatter));
    }
    ...
    Range range =
        new Range(from.withDayOfMonth(1), to.plusMonths(1).withDayOfMonth(1).minusDays(1));

    return new RangeMessageDTO(
        range, Message.create(ErrorType.FILTER, ErrorSeverity.INFO, message));
  }
  ...
  private Filter getFilterForEnhets(
      @Nonnull List<HsaIdEnhet> enhetsAsHsaIds, boolean vardenhetsdepth) {
    final String hashValue =
        FilterPredicates.getHashValueForEnhets(enhetsAsHsaIds) + vardenhetsdepth;
    final FilterPredicates predicate =
        new FilterPredicates(
            fact ->
                enhetsAsHsaIds.contains(
                    vardenhetsdepth ? fact.getVardenhet() : fact.getUnderenhet()),
            sjukfall -> true,
            hashValue,
            false);
  ...
  private Filter getFilterForAllAvailableEnhets(
      HttpServletRequest request, boolean vardenhetsdepth) {
    ...
    if (info.getLoginInfoForVg(vgId).map(LoginInfoVg::isProcessledare).orElse(false)) {
      ...
      return new Filter(
          ...
          null,
          null,
          toReadableSjukskrivningslangdName(null),
          toReadableAgeGroupNames(null),
          FilterPredicates.HASH_EMPTY_FILTER + vardenhetsdepth,
          toReadableIntygTypeName(null),
          true);
    }
    List<HsaIdEnhet> hsaIds =
        info.getBusinessesForVg(vgId).stream().map(Verksamhet::getId).collect(Collectors.toList());
    return getFilterForEnhets(hsaIds, vardenhetsdepth);
  }

  private ArrayList<HsaIdEnhet> getEnhetsFilteredRegion(
      HttpServletRequest request, FilterData inFilter) {
  ...
  private List<String> toReadableAgeGroupNames(List<String> ageGroups) {
    if (ageGroups == null) {
      return Arrays.stream(AgeGroup.values())
          .map(AgeGroup::getGroupName)
          .collect(Collectors.toList());
    }
    return ageGroups.stream()
        .map(
            ageGroup ->
                AgeGroup.parse(ageGroup)
  ...
  private Predicate<Fact> getAldersgruppFilter(List<String> ageGroups) {
    ...
    final List<AgeGroup> filteredAgeGroups =
        ageGroups.stream()
            .map(AgeGroup::parse)
            .filter(Optional::isPresent)
            .map(Optional::get)
    ...
    return fact -> {
      ...
      return filteredAgeGroups.stream()
          .anyMatch(group -> group.getFrom() <= age && group.getTo() >= age);
    };
  }

  // When editing this make sure to also update getEnhetNameMapInclusive
  public Map<HsaIdEnhet, String> getEnhetNameMapExclusive(
      HttpServletRequest request, Collection<HsaIdEnhet> enhetsIDs, boolean vardenhetsdepth) {
    ...
    for (Verksamhet userVerksamhet : info.getBusinessesForVg(vgid)) {
      if (enhetsIDs != null
          && enhetsIDs.contains(userVerksamhet.getId())
          && userVerksamhet.isVardenhet() == vardenhetsdepth) {
  ...
  public Map<HsaIdEnhet, String> getEnhetNameMapInclusive(
      HttpServletRequest request, Collection<HsaIdEnhet> enhetsIDs, boolean vardenhetsdepth) {
    final HsaIdVardgivare vgid = getSelectedVgIdForLoggedInUser(request);
    LoginInfo info = loginServiceUtil.getLoginInfo();
    Map<HsaIdEnhet, String> enheter = new HashMap<>();
    for (Verksamhet userVerksamhet : info.getBusinessesForVg(vgid)) {
      if (enhetsIDs != null
          && enhetsIDs.contains(userVerksamhet.getId())
          && (vardenhetsdepth || !userVerksamhet.isVardenhet())) {
        enheter.put(userVerksamhet.getId(), userVerksamhet.getName());
      }
    }
    return enheter;
  ...
  private Map<HsaIdEnhet, String> getEnhetNameMapRegion(
      HttpServletRequest request, List<HsaIdEnhet> filteredEnhetsIds) {
    final HsaIdVardgivare vgid = getSelectedVgIdForLoggedInUser(request);
    Map<HsaIdEnhet, String> enheter = new HashMap<>();
