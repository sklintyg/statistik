...
public class CasesPerCountyConverter {
  ...
  private static final String H_ANTAL_SJUKFALL_PER_1000_INVANARE =
      MessagesText.REPORT_ANTAL_SJUKFALL_PER_1000_INVANARE;
  ...
  public CasesPerCountyConverter(
      SimpleKonResponse sjukfallPerLan, CountyPopulation countyPopulation, Range range) {
  ...
  private TableData convertToTable() {
    ...
    for (int i = 0; i < resp.getRows().size(); i++) {
      ...
      final KonField populationObject =
          countyPopulation.getPopulationPerCountyCode().get(row.getExtras().toString());

      final int totalNumberOfSjukfall = row.getFemale() + row.getMale();
      final int totalPopulation =
          populationObject == null ? 0 : populationObject.getMale() + populationObject.getFemale();
      final Object totalPopulationValueToShow = populationObject != null ? totalPopulation : "-";
      final Object totalSjukfallPerKPopulation =
          calculateSjukfallPerKPopulation(populationObject, totalNumberOfSjukfall, totalPopulation);
      ...
      final Object femaleSjukfallPerKPopulation =
          calculateSjukfallPerKPopulation(
              populationObject, femaleNumberOfSjukfall, femalePopulation);
      ...
      final Object maleSjukfallPerKPopulation =
          calculateSjukfallPerKPopulation(populationObject, maleNumberOfSjukfall, malePopulation);

      final List<Object> rowData =
          Arrays.asList(
              totalNumberOfSjukfall,
              femaleNumberOfSjukfall,
              maleNumberOfSjukfall,
              totalPopulationValueToShow,
              femalePopulationValueToShow,
              malePopulationValueToShow,
              totalSjukfallPerKPopulation,
              femaleSjukfallPerKPopulation,
              maleSjukfallPerKPopulation);
      if (!Lan.OVRIGT_ID.equals(row.getExtras()) || totalNumberOfSjukfall > 0) {
        data.add(new NamedData(row.getName(), rowData));
      }
    }

    List<TableHeader> headers1 =
        Arrays.asList(
            new TableHeader(""),
            new TableHeader(H_ANTAL_SJUKFALL, COLSPAN),
            new TableHeader(H_ANTAL_INVANARE, COLSPAN),
            new TableHeader(H_ANTAL_SJUKFALL_PER_1000_INVANARE, COLSPAN));
    final List<String> headerTexts =
        Arrays.asList(
            LAN, H_TOTALT, H_KVINNOR, H_MAN, H_TOTALT, H_KVINNOR, H_MAN, H_TOTALT, H_KVINNOR,
            H_MAN);
  ...
  private Object calculateSjukfallPerKPopulation(
      KonField populationObject, int numberOfSjukfall, int population) {
    final boolean calculateThisRow = population != 0 || populationObject != null;
    return calculateThisRow
        ? SjukfallPerPatientsPerEnhetConverter.roundToTwoDecimalsAndFormatToString(
            numberOfSjukfall / (population / THOUSAND))
        : "-";
  }

  private List<Object> getSamtligaLanRowData() {
    final int totalSjukfall =
        resp.getRows().stream().mapToInt(value -> value.getFemale() + value.getMale()).sum();
    final int totalPopulation =
        countyPopulation.getPopulationPerCountyCode().values().stream()
            .mapToInt(value -> value.getFemale() + value.getMale())
            .sum();
    final String totalSjukfallPerKPopulationForAllLan =
        SjukfallPerPatientsPerEnhetConverter.roundToTwoDecimalsAndFormatToString(
            ((float) totalSjukfall) / (totalPopulation / THOUSAND));

    final int femaleSjukfall = resp.getRows().stream().mapToInt(value -> value.getFemale()).sum();
    final int femalePopulation =
        countyPopulation.getPopulationPerCountyCode().values().stream()
            .mapToInt(value -> value.getFemale())
            .sum();
    final String femaleSjukfallPerKPopulationForAllLan =
        SjukfallPerPatientsPerEnhetConverter.roundToTwoDecimalsAndFormatToString(
            ((float) femaleSjukfall) / (femalePopulation / THOUSAND));

    final int maleSjukfall = resp.getRows().stream().mapToInt(value -> value.getMale()).sum();
    final int malePopulation =
        countyPopulation.getPopulationPerCountyCode().values().stream()
            .mapToInt(value -> value.getMale())
            .sum();
    final String maleSjukfallPerKPopulationForAllLan =
        SjukfallPerPatientsPerEnhetConverter.roundToTwoDecimalsAndFormatToString(
            ((float) maleSjukfall) / (malePopulation / THOUSAND));
    ...
    return Arrays.asList(
        ...
        femalePopulation,
        malePopulation,
        totalSjukfallPerKPopulationForAllLan,
        femaleSjukfallPerKPopulationForAllLan,
        maleSjukfallPerKPopulationForAllLan);
  }
  ...
  private ChartData convertToChart() {
    ...
    final List<ChartCategory> groups =
        new ArrayList<>(Lists.transform(chartGroups, ChartCategory::new));
    groups.add(0, new ChartCategory(SAMTLIGA_LAN));
    final double sjukfallPerKPopulationForAllLanFemale =
        getSjukfallPerKPopulationForAllLanFemale(resp.getRows());
    final double sjukfallPerKPopulationForAllLanMale =
        getSjukfallPerKPopulationForAllLanMale(resp.getRows());
    final double sjukfallPerKPopulationForAllLanTotal =
        getSjukfallPerKPopulationForAllLanTotal(resp.getRows());

    List<ChartSeries> series = new ArrayList<>();
    final Stream<Number> sjukfallPerKPopulationStreamFemale =
        chartRows.stream().map(this::getSjukfallPerKPopulationFemale);
    final List<Number> sjukfallPerPopulationFemale =
        Stream.concat(
                Stream.of(sjukfallPerKPopulationForAllLanFemale),
                sjukfallPerKPopulationStreamFemale)
            .collect(Collectors.toList());

    final Stream<Number> sjukfallPerKPopulationStreamMale =
        chartRows.stream().map(this::getSjukfallPerKPopulationMale);
    final List<Number> sjukfallPerPopulationMale =
        Stream.concat(
                Stream.of(sjukfallPerKPopulationForAllLanMale), sjukfallPerKPopulationStreamMale)
            .collect(Collectors.toList());

    final Stream<Number> sjukfallPerKPopulationStreamTotal =
        chartRows.stream().map(this::getSjukfallPerKPopulationTotal);
    final List<Number> sjukfallPerPopulationTotal =
        Stream.concat(
                Stream.of(sjukfallPerKPopulationForAllLanTotal), sjukfallPerKPopulationStreamTotal)
            .collect(Collectors.toList());
  ...
  private double getSjukfallPerKPopulationForAllLanTotal(List<SimpleKonDataRow> rows) {
    ...
    final int totalPopulation =
        countyPopulation.getPopulationPerCountyCode().values().stream()
            .mapToInt(field -> field.getMale() + field.getFemale())
            .sum();
  ...
  private double getSjukfallPerKPopulationTotal(SimpleKonDataRow row) {
    ...
    final int population =
        populationObject == null ? 0 : populationObject.getFemale() + populationObject.getMale();
  ...
  private double calculateSjukfallPerKPopulation(int numberOfSjukfall, int population) {
    ...
    return SjukfallPerPatientsPerEnhetConverter.roundToTwoDecimals(
        (float) numberOfSjukfall / (population / THOUSAND));
  }
  ...
  public CasesPerCountyData convert() {
    ...
    final String originDate =
        countyPopulation.getDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    return new CasesPerCountyData(
        tableData,
        chartData,
        fullRange.toString(),
