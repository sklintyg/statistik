...
options:
    ...
    -f <filename>   Database query result file name
    -b <hostname>   Database host name
    -v <caregiver>  Caregiver ID
...
def get_data(file_name, start, end, caregiver, host, password):
    ...
    sql_cmd = 'mysql --user root --password=%s statistik -e "select * from wideline w where ((startdatum >= %s and startdatum <= %s) or '  % (password, start, end)
...
def main(argv):
    ...
    opts, args = getopt.getopt(argv,"tdapglni:ekKL:hsE:Nj:9:f:v:w:b:")
    for opt, arg in opts:
        if opt == '-t':
            threshold = True
        elif opt == '-e':
    ...
    if result_file:
        if start_interval is None or end_interval is None:
            ...
            return
        if not caregiver:
            print "ERROR: Vårdgivare saknas"
            usage()
            return
        if not db_password or not db_host:
            print "ERROR: Databasparametrar saknas (host/password)"
            usage()
            return
            
        get_data(result_file, start_interval, end_interval, caregiver, db_host, db_password)
        return
