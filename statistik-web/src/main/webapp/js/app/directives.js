...
angular.module('StatisticsApp').directive("submenu", function (recursionService) {
    return {
        ...
        template:
            '<span ng-click="recursionhelper.hideclick(item)" class="ellipsis-text"><span class="glyphicon" ng-class="{glyphiconMinusSign: !item.hideChildren, glyphiconPlusSign: item.hideChildren}"/>{{item.id}} {{item.name}}</span>' +
